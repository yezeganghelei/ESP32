...
/* LCD requires a set of initialization commands/Parameter values. They are stored in this structure  */
typedef struct
{
    uint8_t cmd;
    uint8_t data[16];
...
/**
 * @brief       Send data to LCD, use polling mode to block wait for the transmission to complete (Due to the small amount of data transmission，Therefore, processing in polling mode can increase the speed。The overhead of using interrupt methods exceeds that of polling methods)
...
/**
 * @brief       Filling a single indivual color in a specified area
 * @param       (sx,sy),(ex,ey):Fill the diagonal coordinates of rectangles,The area size is:(ex - sx + 1) * (ey - sy + 1)
 ...
 * @param       dir:0~7,represent8indivual direction (See the specific definition forlcd.h)
 * @retval      none
 */
...
/**
 * @brief       Drawing an individual pixel
...
void lcd_draw_line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
    ...
    if( delta_x>delta_y)
    {
        distance = delta_x;                 /* Select the basic increment coordinateaxis */
    }
    else
    {
        distance = delta_y; 
...
/**
 * @brief       Draw an individual rectangle
 * @param       x1,y1   Starting point coordinates
 * @param       x2,y2   End point coordinates
 * @param       color Fill color
 * @retval      none
...
/**
 * @brief       Draw an indivual circle
 * @param       x0,y0   Center of circlecoordinate
...
/**
 * @brief       Show an individual character at the specified position
...
void lcd_show_char(uint16_t x, uint16_t y, uint8_t chr, uint8_t size, uint8_t mode, uint16_t color)
{
    ...
    uint8_t *pfont = 0;
    uint8_t csize = 0;                                      /* Get the number of bytes occupied by the dot matrix set of the Font-indivual character right */
    uint16_t colortemp = 0;
    uint8_t sta = 0;

    csize = (size / 8 + ((size % 8) ? 1 : 0)) * (size / 2); /* Get the number of bytes occupied by the dot matrix set of the Font-indivual character right */
...
void lcd_show_num(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint16_t color)
{
    ...
    for (t = 0; t < len; t++)                                               /* Cycling by total number of displayed digits */
    {
        ...
        if (enshow == 0 && t < (len - 1))                                   /* There is no enabled display, and there are bits to display */
        {
            if (temp == 0)
            {
                lcd_show_char(x + (size / 2)*t, y, ' ', size, 0, color);    /* show space,Placeholder */
                continue;                                                   /* Continue to the next individual*/
            }
            else
            {
...
/**
 * @brief       Extended displaylenindivual numbers (The high position is 0 is also shown)
 * @param       x,y : starting coordinates
 * @param       num : Value(0 ~ 2^32)
 * @param       len : Display the number of digits
 * @param       size: Select a font 12/16/24/32
 * @param       mode: Display mode
 *              [7]:0,not filling;1,filling0.
 *              [6:1]:reserve
 *              [0]:0,not Overlay display;1,Overlay display.
    ...
    for (t = 0; t < len; t++)                                                           /* Cycling by total number of displayed digits */
    {
        ...
        if (enshow == 0 && t < (len - 1))                                               /* There is no enabled display, and there are bits to display */
        {
            if (temp == 0)
            {
                ...
                {
                    lcd_show_char(x + (size / 2)*t, y, ' ', size, mode & 0X01, color);  /* use space Placeholder */
                }
                continue;
            }
            else
...
#else                                                               /* Not for 0 is regarded as using 1.3inchSPILCDscreen, Then the screen will not display inversely */
    lcd_init_cmd_t ili_init_cmds[] =
    {
        {0x11, {0}, 0x80},
        {0x36, {0x00}, 1},
