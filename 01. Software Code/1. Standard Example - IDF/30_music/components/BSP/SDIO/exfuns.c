...
static const char *FILE_TYPE_TBL[FILE_MAX_TYPE_NUM][FILE_MAX_SUBT_NUM] = {
    ...
    {"TXT", "C", "H"," "," "," "," "},                      /* Text file */
    {"WAV", "MP3", "OGG", "FLAC", "AAC", "WMA", "MID"},     /* music document */
    {"BMP", "JPG", "JPEG", "GIF","PNG"," "," "},            /* picture document */
    {"AVI"," "," "," "," "," "," "},                        /* Video files */
};
...
uint8_t exfuns_init(void)
{
    ...
    {
        fs[i] = (FATFS *)malloc(sizeof(FATFS));   /* for disk i workspace Apply for memory */
...
/**
 * @brief      Will convert lowercase letters to capital letters. If it is a number, stay unchanged.
 ...
 * @retval      File Type
 *   @arg       0XFF , Indicates the file type number identified by the none method.
...
uint8_t exfuns_file_type(char *fname)
{
    ...
    while (i < 250)
    {
        ...
        if (*fname == '\0')break;   /* The offset arrived last.. */
    ...
    for (i = 0; i < 5; i++)     /* arriveSuffix name*/
    {
        fname--;
    ...
    for (i = 0; i < 4; i++)tbuf[i] = exfuns_char_upper(tbuf[i]);    /* All changed to capital */
    ...
    return 0XFF;    /* No arrival found */
}

/**
 * @brief       Get the remaining capacity of the disk
...
uint8_t exfuns_get_free(uint8_t *pdrv, uint32_t *total, uint32_t *free)
{
    ...
    if (res == 0)
    {
        tot_sect = (fs1->n_fatent - 2) * fs1->csize;    /* Get the total number of sectors that arrive */
...
#endif
        *total = tot_sect >> 1;     /* Unit forKB */
        *free = fre_sect >> 1;      /* Unit forKB */
    }
 ...
 *                  pname:document/Folders
 *                  pct:percentage
 *                  mode:
 *                      bit0 : Update document name
 *                      bit1 : Update percentagepct
 *                      bit2 : Update Folders
 *                      other : reserve
 ...
 * @param       totsize : Total size(whentotsizefor0When,Indicates that only a single document copy)
    ...
    else
    {
        ...
        if (res == 0)           /* Both have been successfully opened */
        {
            ...
            while (res == 0)    /* Start copying */
            {
                ...
                if (curpct != (lcpdsize * 100) / totsize)       /* Do you need to update the percentage? */
                {
...
uint8_t *exfuns_get_src_dname(uint8_t *pname)
{
    ...
    while ((*pname != 0x5c) && (*pname != 0x2f))pname--;    /* RecaparriveThe last one"\"or"/"Where */

    return ++pname;
}

/**
 * @brief Get arriveFolders size
 * @note Notice: Folders size should not exceed 4GB..
 * @param pname : Detailed path
 * @retval 0, Folders size for0, or an error occurred during reading.
 * other, Folders size
 */
uint32_t exfuns_get_folder_size(uint8_t *fdname)
{
#define MAX_PATHNAME_DEPTH  512 + 1     /* Maximum target file path + file name depth */
    uint8_t res = 0;
    FF_DIR *fddir = 0;         /* Table of contents */
    FILINFO *finfo = 0;     /* File information */
    uint8_t *pathname = 0;  /* Destination folder path + document name */
        ...
        if (res == 0)
        {
            ...
            if (res == 0)   /* Open directory successfully */
            {
                while (res == 0)   
                {
                    ...
                    if (finfo->fattrib & 0X10)                      /* Subdirectory(File properties,0X20,Archive files;0X10,Subdirectory;) */
                    {
                        ...
                        fdsize += exfuns_get_folder_size(pathname);             /* get arriveSubdirectorysize,Recursive call */
 ...
 * @param       psrc    : source Folders
 * @param       pdst    : Destination folder
 *   @note      must be in the form of"X:"/"X:XX"/"X:XX/XX"Something like that. And make sure that the upper level Folders exist
...
#define MAX_PATHNAME_DEPTH 512 + 1  /* Maximum target file path + file name depth */
    ...
    FILINFO *finfo = 0;     /* File information */
    uint8_t *fn = 0;        /* Long document name */

    uint8_t *dstpathname = 0;   /* Destination folder path + document name*/
    uint8_t *srcpathname = 0;   /* Source Folders path + document name */
    ...
    if (res == 0)
    {
        ...
        if (res == 0)
        {
            ...
            strcat((char *)dstpathname, (const char *)pdst);    /* Copy the original Target file path */
            ...
            if (res == 0)   /* Open directory successfully */
            {
                ...
                fcpymsg(fn, 0, 0X04);   /* Update Folders name*/
                res = f_mkdir((const TCHAR *)dstpathname);  /* If Folders already exists,Do not create.Create a new folder if it does not exist. */

                if (res == FR_EXIST)res = 0;

                while (res == 0)        /* Start copyingFolders inside the East */
                {
                    ...
                    dstpathlen = strlen((const char *)dstpathname); /* Get the length of the target path before arrivewhen */
                    srcpathlen = strlen((const char *)srcpathname); /* Get arriveSource path length */
                    ...
                    if (finfo->fattrib & 0X10)  /* Subdirectory(File properties,0X20,Archive files;0X10,Subdirectory;) */
                    {
                        ...
                        res = exfuns_folder_copy(fcpymsg, srcpathname, dstpathname, totsize, cpdsize, fwmode);   /* copy Folders */
                    }
                    else     /* Non-directory */
                    {
                        strcat((char *)dstpathname, (const char *)"/"); /* Add slashes to target path */
                        ...
                        fcpymsg(fn, 0, 0X01);       /* Update document name */
