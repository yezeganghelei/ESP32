...
/* Font libraryThe total number of sectors occupied by the areasize(3indivualFont library+unigbksurface+Font libraryinformation=3238700 byte,Approximately occupied791sectors,onesectors4Kbyte) */
#define FONTSECSIZE         791
/* EverySecond-rateOperational restrictions are 4K Within */
#define SECTOR_SIZE         0X1000

/* PartitionsurfaceFont libraryStorage startaddress
 * UNIGBK.BIN,Total size3.09M, 791sectors,quiltFont library占用It's,Can't move!
 */
#define FONTINFOADDR        0

/* Used to save Font library basic information, address, Size, etc. */
_font_info ftinfo;
...
/**
 * @brief       PartitionsurfaceRead data
 * @param       buffer    : Read dataStorage area
 ...
 * @param       offset    : Write datastarting address
...
/**
 * @brief       显Show当前字体renewschedule
 * @param       x, y    : coordinate
 * @param       size    : Font size
 * @param       totsize : The entire file size
    ...
    if (rval == 0)
    {
        switch (fx)
        {
            case 0: /* renew UNIGBK.BIN */
                ftinfo.ugbkaddr = FONTINFOADDR + sizeof(ftinfo);    /* After the header，FollowUNIGBKConvert codesurface */
            ...
            case 3: /* renew GBK24.BIN */
                ...
                flashaddr = ftinfo.f24addr;                         /* GBK24starting address */
                break;
        }
...
uint8_t fonts_init(void)
{
    ...
    while (t < 10)  /* Continuous reading10Second-rate,All are wrong,It means there is indeed a problem,have torenewFont libraryIt's */
    {
        t++;
