...
uint8_t exfuns_file_type(char *fname)
{
    ...
    for (i = 0; i < 4; i++)tbuf[i] = exfuns_char_upper(tbuf[i]);    /* All changedforcapital */
                                      ...
                                      uint32_t totsize, uint32_t cpdsize, uint8_t fwmode)
{
    ...
    else
    {
        ...
        {
            if (totsize == 0)   /* Copy a single file only */
            {
            ...
            while (res == 0)    /* Start copying */
            {
                ...
                if (curpct != (lcpdsize * 100) / totsize)       /* Is it necessary to updatepercentage */
                {
 ...
 *              other, first address of the folder name
 */
 ...
 *   @note      Note: the folder size must not exceed 4 GB.
 * @param       pname : Detailed path 
...
#define MAX_PATHNAME_DEPTH  512 + 1     /* maximumTarget filepath+File name depth */
    ...
    if (res == 0)
    {
        ...
        if (res == 0)
        {
            ...
            {
                while (res == 0)   /* Start copyingFoldersstuff inside */
                {
                    ...
                    if (finfo->fattrib & 0X10)                      /* yesSubdirectory(File properties,0X20,Archive files;0X10,Subdirectory;) */
                    {
                        ...
                        pathname[pathlen] = 0;                                  /* Join End symbol */
                    }
                    else
                    {
        ...
        if (res == 0)
        {
            ...
            if (res == 0)   /* Open directory successfully */
            {
                ...
                while (res == 0)        /* Start copyingFoldersstuff inside */
                {
                    ...
                    dstpathlen = strlen((const char *)dstpathname); /* Get the length of the current destination path */
                    ...
                    srcpathname[srcpathlen] = 0;    /* Join End symbol */
                    dstpathname[dstpathlen] = 0;    /* Join End symbol */
                }
            }
