...
#define WAV_TX_BUFSIZE    8192  /* Define the WAV TX DMA array size (when playing 192Kbps@24bit, you need to set 8192 large to avoid jamming) */
...
typedef struct
{
    uint32_t ChunkID;           /* chunk id; here is fixed to "fmt", which is 0X20746D66 */
    uint32_t ChunkSize ;        /* Subset size(Not includedIDandSize);Here is:20. */
...
typedef struct 
{
    ...
    uint32_t ChunkSize ;        /* Subset size(Not includedIDandSize);Here is:4. */
...
typedef struct
{
    ...
    uint32_t ChunkSize ;        /* Subset size(Not includedIDandSize) */
}ChunkDATA;     /* Data block */

typedef struct
{ 
    ChunkRIFF riff;             /* riffpiece */
    ChunkFMT fmt;               /* fmt block */
//    ChunkFACT fact;           /* factpiece LinearPCM,There is no such structure */
    ChunkDATA data;             /* Data block */
}__WaveHeader;  /* wav head */
...
uint8_t wav_play_song(uint8_t *fname);                              /* Play a certainWAVdocument */
#endif