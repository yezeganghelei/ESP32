...
{
    g_audiodev.status = 3 << 0; /* Start playback + non-pause */
    i2s_trx_start();
}
...
/**
 * @brief Get the total number of target files under the path path
...
uint16_t audio_get_tnum(uint8_t *path)
{
    ...
    if ((res == FR_OK) && tfileinfo)
    {
        while (1)                                               /* Query the total number of valid files */
        {
            ...
            {
                break;                                          /* Error/It's at the end, exit */
            }
 ...
 * @param       bitrate: Bit rate(Position speed)
 * @retval      none
 */
...
void audio_play(void)
{
    ...
    es8388_adda_cfg(1, 0);                                      /* OpenDACclosureADC */
    es8388_output_cfg(1, 1);                                    /* DACSelect a channel1output */

    while (f_opendir(&wavdir, "0:/MUSIC"))                      /* Open the music folder */
    {
        text_show_string(30, 190, 240, 16, "MUSICFolder Error!", 16, 0, BLUE);
        vTaskDelay(200);
    ...
    while (res == FR_OK)                                        /* Open successfully */
    {
        ...
        else if (key == KEY0_PRES)                              /* Next song */
        {
            ...
            {
                curindex = 0;                                   /* to the end,Automatically start from scratch */
            }
        }
        else
        {
