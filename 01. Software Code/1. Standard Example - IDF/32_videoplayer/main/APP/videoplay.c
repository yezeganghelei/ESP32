...
static uint16_t video_get_tnum(char *path)
{
    ...
    res = (uint8_t)f_opendir(&tdir, (const TCHAR *)path);       /* Open directory */
    
    if ((res == 0) && tfileinfo)
    {
        while (1)                                               /* Count the total number of valid files */
        {
            res = (uint8_t)f_readdir(&tdir, tfileinfo);         /* Read one file in the directory */
            
            if ((res != 0) || (tfileinfo->fname[0] == 0))       /* Error or end of directory, quit */
            {
                break;
            }
            ...
            if ((res & 0xF0) == 0x60)                           /* Is a video file */
            {
                rval++;
            }
        }
...
//    text_show_string(10, 10, lcd_self.width - 10, 16, (char *)buf, 16, 0, RED);   /* Display document name */
    
    sprintf((char *)buf, "index:%d/%d", index, total);
//    text_show_string(10, 30, lcd_self.width - 10, 16, (char *)buf, 16, 0, RED);   /* Display index */
    
    free(buf);                                                        /* Free memory */
}

/**
 * @brief       Display information about the current video file
 ...
 * @param       favi   : Video file currently being played
 * @param       aviinfo: aviControl structure
 * @retval      none
 */
void video_time_show(FIL *favi, AVI_INFO *aviinfo)
{
    ...
    uint32_t totsec = 0;                                            /* Total video time */
...
static uint8_t video_play_mjpeg(uint8_t *pname)
{
    ...
    while (res == 0)
    {
        res = (uint8_t)f_open(favi, (const TCHAR *)pname, FA_READ);                                                     /* Open file */
        ...
        if (res == 0)
        {
            ...
            f_lseek(favi, offset + 12);                                                                                 /* Skip the signature ID; read address is offset to the start of the stream data */
            ...
            if (g_avix.SampleRate)                                          /* Initialize only if audio information is present */
            {
                printf("g_avix.SampleRate:%ld\r\n",g_avix.SampleRate);
                es8388_sai_cfg(0, 3);                                       /* Philips standard, 16-bit data length */
                i2s_set_samplerate_bits_sample(g_avix.SampleRate,I2S_BITS_PER_SAMPLE_16BIT);    /* Set sampling rate */
                i2s_start(I2S_NUM);
            }
            ...
            while (1)
            {
                ...
                {
                    pbuf = framebuf;
                    f_read(favi, pbuf, g_avix.StreamSize + 8, (UINT*)&nr);  /* Read the whole frame + next frame data stream ID information */
                    ...
                    while (frameup == 0);                                   /* Wait until the playback time is reached */
                    frameup = 0;
                }
                else
                {
                    video_time_show(favi, &g_avix);                         /* Display current playback time */
                ...
                if (avi_get_streaminfo(pbuf + g_avix.StreamSize) != 0)      /* Read the next frame stream tag */
                {
...
void video_play(void)
{
    ...
    while (f_opendir(&vdir, "0:/VIDEO") != FR_OK)                               /* Check whether the VIDEO folder exists */
    {
        text_show_string(60, 190, 240, 16, "VIDEO folder error!", 16, 0, RED);
        vTaskDelay(200);
        lcd_fill(60, 190, 240, 206, WHITE);
        vTaskDelay(200);
    }
    
    totavinum = video_get_tnum("0:/VIDEO");                                     /* Check whether there are video files */
    
    while (totavinum == 0)
    {
        text_show_string(60, 190, 240, 16, "No video file!", 16, 0, RED);
        vTaskDelay(200);
    ...
    res = (uint8_t)f_opendir(&vdir, "0:/VIDEO");                                /* Open directory */
    ...
    if (res == 0)
    {
        ...
        while (1)
        {
            ...
            if ((res != 0) || (vfileinfo->fname[0] == 0))                       /* Error or end of directory, quit */
            {
                break;
            }
    ...
    while (res == 0)
    {
        dir_sdi(&vdir, voffsettbl[curindex]);                                   /* Change the current directory index */
        ...
        strcat((char *)pname, (const char *)vfileinfo->fname);                  /* Append the file name */
        lcd_clear(WHITE);
 ...
 * @param       favi    : AVI file
 * @param       aviinfo : AVI information structure
...
uint8_t video_seek(FIL *favi, AVI_INFO *aviinfo, uint8_t *mbuf)
{
    ...
    while (1)
    {
        ...
        if (fpos == 0)                                     /* Starting from 0, must first look for the movie ID */
        {
        ...
        avi_get_streaminfo(pbuf + offset);                  /* Get stream information */
