...
/**
 * @brief       Send data toLCD，makeusePolling method blockingwaitTo be transferred(Due to the small amount of data transmission，Therefore, processing in polling mode can increase the speed。The overhead of using interrupt methods exceeds that of polling methods)
...
/**
 * @brief filling a single individual color in the specified area
...
void lcd_show_char(uint16_t x, uint16_t y, uint8_t chr, uint8_t size, uint8_t mode, uint16_t color)
{
    ...
    uint8_t *pfont = 0;
    uint8_t csize = 0;                                      /* getFontoneindivualcharacterrightThe number of bytes occupied by the corresponding lattice set */
    uint16_t colortemp = 0;
    uint8_t sta = 0;

    csize = (size / 8 + ((size % 8) ? 1 : 0)) * (size / 2); /* getFontoneindivualcharacterrightThe number of bytes occupied by the corresponding lattice set */
