...
uint8_t exfuns_get_free(uint8_t *pdrv, uint32_t *total, uint32_t *free)
{
    ...
    if (res == 0)
    {
        tot_sect = (fs1->n_fatent - 2) * fs1->csize;    /* have toarriveTotal number of sectors */
 ...
 * @param       totsize : Total size(whentotsizefor0When,means onlyforsingledocumentcopy)
