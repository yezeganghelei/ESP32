...
#define BRRED           0XFC07      /* Brownred */
#define GRAY            0X8430      /* grey */ 
#define DARKBLUE        0X01CF      /* Dark blue */
#define LIGHTBLUE       0X7D7C      /* shallowblue */ 
#define GRAYBLUE        0X5458      /* Ashblue */ 
#define LIGHTGREEN      0X841F      /* shallowgreen */  
#define LGRAY           0XC618      /* shallow grey(PANNEL),Form background color */ 
...
typedef struct _lcd_obj_t
{
    ...
    uint16_t        setxcmd;        /* set upxcoordinateinstruction */
    uint16_t        setycmd;        /* set upy coordinate instruction */
...
void lcd_write_data(const uint8_t *data, int len);                                                                      /* senddata到LCD */
...
void lcd_show_string(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t size, char *p, uint16_t color);   /* show字符串 */
...
void lcd_show_char(uint16_t x, uint16_t y, uint8_t chr, uint8_t size, uint8_t mode, uint16_t color);                    /* In the specified locationshowOne character */
