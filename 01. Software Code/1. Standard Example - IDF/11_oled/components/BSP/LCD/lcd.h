...
#define MAGENTA         0XF81F      /* Magenta = BLUE + RED */
...
#define LGRAY           0XC618      /* Light gray (panel), used as background color */ 
#define LGRAYBLUE       0XA651      /* Light gray-blue (intermediate layer color) */ 
#define LBBLUE          0X2B12      /* Light brown-blue (reverse color for selection) */ 
...
typedef struct _lcd_obj_t
{
    ...
    uint8_t         dir;            /* Portrait/landscape control: 0 = portrait, 1 = landscape */
    uint16_t        wramcmd;        /* Start GRAM write command */
...
/* LCD buffer size settings; take care when modifying these values! Changing them may affect lcd_clear/lcd_fill/lcd_draw_line */
...
void lcd_write_data(const uint8_t *data, int len);                                                                      /* Send data to the LCD */
void lcd_write_data16(uint16_t data);                                                                                   /* Send 16-bit data to the LCD */
...
void lcd_show_num(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint16_t color);                     /* Display len numbers */
void lcd_show_xnum(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint8_t mode, uint16_t color);      /* Display len numbers with leading zeros */
...
void lcd_draw_line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,uint16_t color);                                  /* Line drawing function (horizontal/vertical and diagonal lines) */
void lcd_draw_pixel(uint16_t x, uint16_t y, uint16_t color);                                                            /* Draw a pixel */
void lcd_show_char(uint16_t x, uint16_t y, uint8_t chr, uint8_t size, uint8_t mode, uint16_t color);                    /* Display one character at the specified position */
