...
/**
 * @brief       Send data toLCD，makeusePolling method blockingwaitTo be transferred(Due to the small amount of data transmission，Therefore, the process in the polling method can improve the speed。The overhead of using interrupt methods exceeds that of polling methods)
...
/**
 * @brief       set upLCDAutomatic scanning direction(rightRGBInvalid screen)
...
void lcd_draw_line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
    ...
    {
        lcd_draw_pixel(urow,ucol,color);    /* Draw points */ 
        xerr += delta_x ; 
        yerr += delta_y ; 
...
/**
 * @brief       at specified locationshowoneindivualcharacter
...
void lcd_show_char(uint16_t x, uint16_t y, uint8_t chr, uint8_t size, uint8_t mode, uint16_t color)
{
    ...
    chr = chr - ' ';                                        /* Get the offset value（ASCIIFont library starts with spaces to get the modulus，so-' 'It is the font library for corresponding characters） */
