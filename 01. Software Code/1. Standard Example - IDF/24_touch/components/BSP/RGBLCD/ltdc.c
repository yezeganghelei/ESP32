...
void ltdc_init(void)
{
    ...
    esp_lcd_rgb_panel_config_t panel_config = { /* RGBLCDConfigure structure */
        ...
        .timings = {                            /* RGBLCDhour sequence parameter */
            ...
            .hsync_front_porch = ltdcdev.hfp,   /* Horizontal front porch，ActivitynumberEndandNexthsyncBetweenPCLKnumber */
 ...
 * @note This function number only supports uint16_t, RGB565 format of color number group filling.
 * (sx,sy),(ex,ey):Fill the diagnostic coordinates of rectangles,Area size is:(ex - sx + 1) * (ey - sy + 1)
 ...
 * @param       x,y,width,height:Coordinates and rulersinch
...
/**
 * @brief exist specifies that Bit sets a character to show
 * @param x,y :Coordinates
 * @param chr :Characters to be displayed:" "--->"~"
 * @param size :Font size 12/16/24/32
 * @param mode :Overlay method(1); non-Overlay method(0);
    ...
    for (t = 0; t < csize; t++)
    {
        ...
        for (t1 = 0; t1 < 8; t1++)                          /* One byte8One dot */
        {
            ...
            if ((y - y0) == size)                           /* A column has been displayed? */
            {
                ...
                x++;                                        /* x coordinate increments */
                
                if (x >= ltdcdev.width)
                {
                    return;                                 /* xThe coordinates exceed the area */
                }
...
void ltdc_show_num(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint16_t color)
{
    ...
    for (t = 0; t < len; t++)                                               /* Loop by total showBitnumber */
    {
        ...
        if (enshow == 0 && t < (len - 1))                                   /* There is no show enabled, and there is still Bit to show */
        {
            if (temp == 0)
            {
 ...
 *              [6:1]:reserve
 *              [0]:0,non-superpositionshow;1,Overlayshow.
 * @param       color   :Font color
 * @retval      none
 */
