...
i2c_obj_t iic_init(uint8_t iic_port)
{
    ...
    iic_master[i].init_flag = i2c_driver_install(iic_master[i].port,        /* Port number */
                                                 ...
                                                 I2C_MASTER_TX_BUF_DISABLE, /* From the machinemodelSend buffer size(Host mode is not used) */     
 ...
 * @param       n   ：Data size
 * @param       bufs：The data to be sent is eitherreadThe storage area to be picked
 * @param       flags：readWrite logo bit
 * @retval      none
 */
