...
/**
 * @brief Send a command to the LCD, blocking in polling mode until transmission completes (the data volume is very small, so polling is faster; interrupt overhead exceeds polling overhead)
...
/**
 * @brief       Send data to the LCD, blocking in polling mode until transmission completes (the data volume is very small, so polling is faster; interrupt overhead exceeds polling overhead)
...
{	
    ...
    lcd_write_cmd(lcd_self.wramcmd);    /* Start writing GRAM */
}   

/**
 * @brief       Clear the LCD screen with one color
 ...
 * @param (sx,sy),(ex,ey): rectangle corner coordinates; area size is (ex - sx + 1) * (ey - sy + 1)
 * @param color: fill color (32-bit color, for LTDC compatibility)
 * @retval none
 */
...
/**
 * @brief       Set the LCD automatic scanning direction (invalid for RGB screens)
 * @param       dir: 0~7, representing 8 directions (see lcd.h for details)
 * @retval      none
 */
void lcd_scan_dir(uint8_t dir)
{
    ...
    /* In landscape mode, do not change the scanning direction for 1963 but change it for other ICs; in portrait mode, change the direction for 1963 but not for other ICs */
    if (lcd_self.dir == 1)
    {
        dir = 5;
    }

    /* Set bits 5, 6, 7 of the 0X36 register according to the scanning direction */
    switch (dir)
    {
    ...
    dirreg = 0x36;                              /* For most driver ICs, controlled by the 0X36 register */
...
/**
 * @brief       Draw a line (straight or diagonal)
...
void lcd_draw_line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
    ...
    delta_x = x2 - x1;                      /* Calculate the coordinate increment */
    delta_y = y2 - y1; 
    urow = x1; 
    ucol = y1; 
    ...
    if( delta_x>delta_y)
    {
        distance = delta_x;                 /* Select the base increment axis */
    }
    else
    {
        distance = delta_y; 
...
void lcd_show_char(uint16_t x, uint16_t y, uint8_t chr, uint8_t size, uint8_t mode, uint16_t color)
{
    ...
    uint8_t *pfont = 0;
    uint8_t csize = 0;                                      /* Number of bytes occupied by the font data for one character */
    uint16_t colortemp = 0;
    uint8_t sta = 0;

    csize = (size / 8 + ((size % 8) ? 1 : 0)) * (size / 2); /* Number of bytes occupied by the font data for one character */
    chr = chr - ' ';                                        /* Get the offset value (the ASCII font library starts with the space character, so subtracting ' ' gives the corresponding character's font data) */
    ...
    {
        case 12:
            pfont = (uint8_t *)asc2_1206[chr];              /* Use the 1206 font */
            break;
...
/**
 * @brief       Display len digits including leading zeros
...
void lcd_show_xnum(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint8_t mode, uint16_t color)
{
    ...
    for (t = 0; t < len; t++)                                                           /* Cycling by total number of displayed digits */
    {
        ...
        if (enshow == 0 && t < (len - 1))                                               /* There is no enabled display, and there are bits to display */
        {
            if (temp == 0)
            {
                if (mode & 0X80)                                                        /* Leading digits need zero padding */
                {
                    lcd_show_char(x + (size / 2)*t, y, '0', size, mode & 0X01, color);  /* Use '0' as placeholder */
                }
                else
                {
                    lcd_show_char(x + (size / 2)*t, y, ' ', size, mode & 0X01, color);  /* Use a space as placeholder */
                }
                continue;
            }
            else
...
void lcd_show_string(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t size, char *p, uint16_t color)
{
    ...
    while ((*p <= '~') && (*p >= ' '))   /* Check whether it is a valid character */
    {
        if (x >= width)
        {
            x = x0;
...
void lcd_init(void)
{
...
#else                                                               /* Otherwise, treat it as a 1.3-inch SPI LCD screen so the display is not inverted */
    lcd_init_cmd_t ili_init_cmds[] =
    {
        {0x11, {0}, 0x80},
        {0x36, {0x00}, 1},
    ...
    /* Loop to send all register settings */
