/**
 ******************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
 * @file camera.c
 * @author
 * @version V1.0
 * @date 2023-08-26
 * @brief camera driver code

 * @license Copyright (c) 2020-2032,
 ******************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
 * @attention

 ******************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
 */

#include "camera.h"

/* Camera configuration */
static camera_config_t camera_config = {
    /* Pin Configuration */
    .pin_pwdn = CAM_PIN_PWDN,
    .pin_reset = CAM_PIN_RESET,
    .pin_xclk = CAM_PIN_XCLK,
    .pin_sccb_sda = CAM_PIN_SIOD,
    .pin_sccb_scl = CAM_PIN_SIOC,
    .pin_d7 = CAM_PIN_D7,
    .pin_d6 = CAM_PIN_D6,
    .pin_d5 = CAM_PIN_D5,
    .pin_d4 = CAM_PIN_D4,
    .pin_d3 = CAM_PIN_D3,
    .pin_d2 = CAM_PIN_D2,
    .pin_d1 = CAM_PIN_D1,
    .pin_d0 = CAM_PIN_D0,
    .pin_vsync = CAM_PIN_VSYNC,
    .pin_href = CAM_PIN_HREF,
    .pin_pclk = CAM_PIN_PCLK,
    /* Image configuration */
    .xclk_freq_hz = 24000000,
    .ledc_timer = LEDC_TIMER_0,
    .ledc_channel = LEDC_CHANNEL_0,
    .fb_location = CAMERA_FB_IN_PSRAM,
    .pixel_format = PIXFORMAT_JPEG,         /* Image output mode */
    .frame_size = FRAMESIZE_QVGA,           /* Image output size */
    .jpeg_quality = 12,                     /* 0-63. For OV series camera sensors, the smaller the quantity means the higher the quality */
    .fb_count = 2,                          /* When usingjpegIn mode，iffb_countMore than one，Then the driver will work in continuous mode */
    .grab_mode = CAMERA_GRAB_WHEN_EMPTY,
};

/**
 * @brief       Camera initialization
 * @param       cmd Transmitted8bit commanddata
 * @retval      none
 */
uint8_t camera_init(void)
{
    esp_err_t err = ESP_OK;

    if (CAM_PIN_PWDN == GPIO_NUM_NC)
    {
        CAM_PWDN(0);
    } 

    if (CAM_PIN_RESET == GPIO_NUM_NC)
    { 
        CAM_RST(0);
        vTaskDelay(20);
        CAM_RST(1);
        vTaskDelay(20);
    }

    /* Camera initialization */
    err = esp_camera_init(&camera_config);

    if (err != ESP_OK)
    {
        return 1;
    }

    sensor_t * s = esp_camera_sensor_get();

    s->set_brightness(s, 0);      /* Set brightness (-2 ~ 2) */
    s->set_contrast(s, 0);        /* Set the contrast (-2 ~ 2) */
    s->set_saturation(s, 0);      /* Set saturation (-2 ~ 2) */
    s->set_hmirror(s, 0);         /* No horizontal flip */

    /* If the camera module isOV3660OrOV5640，The following configuration is required */
    if (s->id.PID == OV3660_PID)
    {
        s->set_vflip(s, 1);         /* Flip backward */
        s->set_brightness(s, 1);    /* Improved brightness */
        s->set_saturation(s, -2);   /* Reduce saturation */
    }
    else if (s->id.PID == OV5640_PID)
    {
        s->set_vflip(s, 1);         /* Flip backward */
    }

    return err;
}

unsigned long i = 0;
unsigned long j = 0;
camera_fb_t *fb = NULL;

/**
 * @brief       Show cameradata（RGB565）
 * @param       x：xAxis coordinates
 * @param       y：yAxis coordinates
 * @retval      none
 */
void camera_show(uint16_t x, uint16_t y)
{
    fb = esp_camera_fb_get();

    if (x + fb->width > lcd_self.width || y + fb->height > lcd_self.height)
    {
        printf("Display area size screen area！！！\r\n");
        goto err;
    }

    lcd_set_window(x, y, x + fb->width - 1, y + fb->height - 1);

    /* lcd_bufStorage camera one frameRGBdata */
    for (j = 0; j < fb->width * fb->height; j++)
    {
        lcd_buf[2 * j] = (fb->buf[2 * i]) ;
        lcd_buf[2 * j + 1] =  (fb->buf[2 * i + 1]);
        i ++;
    }
    
    /* For example：96*96*2/1536 = 12;point12SendRGBdata */
    for(j = 0; j < (fb->width * fb->height * 2 / LCD_BUF_SIZE); j++)
    {
        /* &lcd_buf[j * LCD_BUF_SIZE] Offset address senddata */
        lcd_write_data(&lcd_buf[j * LCD_BUF_SIZE] , LCD_BUF_SIZE);
    }
err:
    esp_camera_fb_return(fb);
    i = 0;
    fb = NULL;
}