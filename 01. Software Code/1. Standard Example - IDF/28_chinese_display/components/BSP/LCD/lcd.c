...
/* LCD需要initializationone组命令/Parameter value。They are stored in this structure  */
typedef struct
{
    uint8_t cmd;
    uint8_t data[16];
...
/**
 * @brief       Send data toLCD，makeusePolling mode blockingwaitWaiting for transfer to be completed(Due to the small amount of data transmission，Therefore, processing in polling mode can increase the speed。The overhead of using interrupt methods exceeds that of polling methods)
