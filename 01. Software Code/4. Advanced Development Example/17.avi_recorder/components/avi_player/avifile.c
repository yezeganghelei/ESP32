...
static int Strl_Parser(const uint8_t *buffer, uint32_t length, uint32_t *list_length)
{
    ...
    *list_length = strl->size + 8; //return the entire size of list
    ...
    if(vids_ID == strh->fourcc_type) {
        ...
        printf("The size of this structure:%d\r\n", strf->size1);
        ...
        printf("Number of palette colors used: %d\r\n", strf->num_colors);
        ...
        printf("strfData block information(audio stream):");
...
int AVI_Parser(const uint8_t *buffer, uint32_t length)
{
    ...
    AVI_file.RIFFchunksize = riff->size; //RIFF data block length
    pdata += sizeof(AVI_LIST_HEAD);
    ...
    AVI_file.LISTchunksize = list->size; //LIST data block length
    pdata += sizeof(AVI_LIST_HEAD);
    ...
    AVI_file.avihsize = avih->size; //avih data block length
#ifdef DEBUGINFO
    printf("-----avih info------\r\n");
    ...
    if ((avih->width > 800) || (avih->height > 480)) {
        ...
        return -6;    //Video size is not supported
    }
    ...
    AVI_file.movi_size = movi->size; //LIST data block length
    pdata += sizeof(AVI_LIST_HEAD);
