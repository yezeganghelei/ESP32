...
static void ili9341_set_orientation(uint8_t orientation)
{
    ...
    uint8_t data[] = {0x48, 0x88, 0x20, 0xE8};
#endif

    ESP_LOGI(TAG, "0x36 command value: 0x%02X", data[orientation]);
    if (1)
    {
        data[orientation] |= MADCTL_BGR;
    }
    else
    {
        data[orientation] |= MADCTL_RGB;
    }
    ili9341_send_cmd(0x36);
    ili9341_send_data((void *)&data[orientation], 1);

    // uint8_t data_[] = {0x01, 0x00, 0x20};
    // ili9341_send_cmd(0xf6);
    // ili9341_send_data((void *)&data_, 3);
}