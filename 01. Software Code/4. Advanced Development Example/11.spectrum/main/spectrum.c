...
//
/* Show frequency table freq/40 becausefftThe interval of the resulting spectrum is40Hz */
...
static int app_exit_flag = 0; //appexit sign
...
/**
 * @brief fftaccumulation function
 * @param fft_bin original data array from start to Finish
...
void generate_spectrum(float *fft_buf, int32_t len)
{
    ...
    for (int i = 0; i < NPT; i++)
    {
        ...
        // amp = fft_buf[index]; // Get the result of fft operation
