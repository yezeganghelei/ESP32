...
float cur_height_arr[NPT] = {0}; // current altitude
float target_height[NPT] = {0};  // target height
...
/* color gradient */
color_gradient_t gradientColor1; //color gradient
...
void spectrum_gui_load(lv_scr_load_anim_t anim_type)
{
    ...
    /* Create a spectrum drawing object */
    ...
    /* Create a background animation draw object */
    ...
    if (anim_type != LV_SCR_LOAD_ANIM_NONE)
    {
        ...
        while (lv_anim_count_running() != 0) //The current page has a long text scroll animation
        {
            lv_task_handler();
            vTaskDelay(1);
        }
...
static float specturm_move_path(float cur, float target)
{
    float val = cur;
    float temp;
    temp = target - cur; //Calculate the current difference
...
static void spectrum_draw_event_cb(lv_event_t *e)
{
    ...
    else if (code == LV_EVENT_DRAW_POST)
    {
        ...
        for (i = 0; i < NPT; i++)
        {
            ...
            new_height = specturm_move_path(cur_height, target_height[i]); //gain new heights
...
{
    static uint16_t spectrum_update_timer_cnt = 0;
    if(spectrum_update_timer_cnt % 1 == 0) // Update background animation every 40ms
    {
