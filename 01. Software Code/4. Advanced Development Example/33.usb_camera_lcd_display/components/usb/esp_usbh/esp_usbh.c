...
static const char *TAG = "ESP_USBH";
...
static esp_err_t esp_usbh_port_event_wait(usbh_port_handle_t port_hdl,
 ...
 */
usbh_port_handle_t esp_usbh_port_init(usbh_port_config_t *config)
{
...
esp_err_t esp_usbh_port_start(usbh_port_handle_t port_hdl)
{
    ERR_CHECK(port_hdl != NULL, "invalid port handle", ESP_ERR_INVALID_ARG);
...
esp_err_t esp_usbh_port_stop(usbh_port_handle_t port_hdl)
{
    ERR_CHECK(port_hdl != NULL, "invalid port handle", ESP_ERR_INVALID_ARG);
    ERR_CHECK(_usbh_driver_installed() == true, "usb port not init", ESP_ERR_INVALID_STATE);
    _usbh_port_t *port_instance = (_usbh_port_t *)port_hdl;
...
esp_err_t esp_usbh_port_deinit(usbh_port_handle_t port_hdl)
{
    ERR_CHECK(port_hdl != NULL, "invalid port handle", ESP_ERR_INVALID_ARG);
    ERR_CHECK(_usbh_driver_installed() == true, "usb port not init", ESP_ERR_INVALID_STATE);
    _usbh_port_t *port_instance = (_usbh_port_t *)port_hdl;
    ...
    esp_usbh_port_event_wait(port_instance, PORT_EVENT_DISCONNECTION, pdMS_TO_TICKS(TIMEOUT_USBH_PW_OFF_MS));
...
void *esp_usbh_port_get_context(usbh_port_handle_t port_hdl)
{
...
static esp_err_t esp_usbh_pipe_event_wait(usbh_pipe_handle_t pipe_hdl,
...
usbh_pipe_handle_t esp_usbh_pipe_init(usbh_port_handle_t port_hdl, const usb_ep_desc_t *ep_desc, QueueHandle_t queue_hdl, void *context)
{
    ERR_CHECK(port_hdl != NULL, "invalid args", NULL);
...
static esp_err_t esp_usbh_pipe_command(usbh_pipe_handle_t pipe_hdl, hcd_pipe_cmd_t command)
{
    ERR_CHECK(pipe_hdl != NULL, "invalid args", ESP_ERR_INVALID_ARG);
    ERR_CHECK(_usbh_driver_installed() == true, "usb port not init", ESP_ERR_INVALID_STATE);
    _usbh_pipe_t *pipe_instance = (_usbh_pipe_t *)pipe_hdl;
...
esp_err_t esp_usbh_pipe_deinit(usbh_pipe_handle_t pipe_hdl)
{
    ERR_CHECK(pipe_hdl != NULL, "invalid args", ESP_ERR_INVALID_ARG);
    ERR_CHECK(_usbh_driver_installed() == true, "usb port not init", ESP_ERR_INVALID_STATE);
    _usbh_pipe_t *pipe_instance = (_usbh_pipe_t *)pipe_hdl;
    ...
    esp_err_t ret = esp_usbh_pipe_command(pipe_hdl, HCD_PIPE_CMD_HALT);
    if (ESP_OK != ret) {
...
void *esp_usbh_pipe_get_context(usbh_pipe_handle_t pipe_hdl)
{
...
esp_err_t esp_usbh_pipe_flush(usbh_pipe_handle_t pipe_hdl, size_t urb_num)
{
    ERR_CHECK(pipe_hdl != NULL, "invalid args", ESP_ERR_INVALID_ARG);
    ERR_CHECK(_usbh_driver_installed() == true, "usb port not init", ESP_ERR_INVALID_STATE);
    _usbh_pipe_t *pipe_instance = (_usbh_pipe_t *)pipe_hdl;
    ...
    ret = esp_usbh_pipe_command(pipe_hdl, HCD_PIPE_CMD_FLUSH);
    if (ESP_OK != ret) {
        ESP_LOGW(TAG, "pipe=%p flush failed", pipe_hdl);
    }
    esp_usbh_pipe_event_wait(pipe_hdl, PIPE_EVENT_URB_DONE, pdMS_TO_TICKS(TIMEOUT_USBH_CTRL_XFER_QUICK_MS));

    for (size_t i = 0; i < urb_num; i++) {
        size_t transfer_num = 0;
        usb_transfer_status_t status = USB_TRANSFER_STATUS_ERROR;
        urb_t *urb = esp_usbh_urb_dequeue(pipe_hdl, &transfer_num, &status);
        ESP_LOGD(TAG, "urb dequeue handle = %p", urb);
    }

    ret = esp_usbh_pipe_command(pipe_hdl, HCD_PIPE_CMD_CLEAR);
    if (ESP_OK != ret) {
...
/*------------------------------------------------ USB URB Code ----------------------------------------------------*/
esp_usbh_urb_handle_t esp_usbh_urb_alloc(int num_isoc_packets, size_t packet_data_buffer_size, void *context)
{
    ...
    return (esp_usbh_urb_handle_t)urb;
}

esp_err_t esp_usbh_urb_free(esp_usbh_urb_handle_t urb_hdl)
{
...
void *esp_usbh_urb_buffer_claim(esp_usbh_urb_handle_t urb_hdl, size_t *buf_size, size_t *num_isoc)
{
...
esp_err_t esp_usbh_urb_enqueue(usbh_pipe_handle_t pipe_hdl, esp_usbh_urb_handle_t urb_hdl, size_t xfer_size)
{
...
esp_usbh_urb_handle_t esp_usbh_urb_dequeue(usbh_pipe_handle_t pipe_hdl, size_t *xfered_size, usb_transfer_status_t *status)
{
    ERR_CHECK(pipe_hdl != NULL, "invalid args", NULL);
    ERR_CHECK(_usbh_driver_installed() == true, "usb port not init", NULL);
    _usbh_pipe_t *pipe_instance = (_usbh_pipe_t *)pipe_hdl;
    ...
    return (esp_usbh_urb_handle_t)_urb;
}

esp_usbh_urb_handle_t esp_usbh_urb_ctrl_xfer(usbh_port_handle_t port_hdl, esp_usbh_urb_handle_t urb_hdl, size_t xfer_size, size_t *xfered_size, usb_transfer_status_t *status)
{
    ...
    esp_err_t ret = esp_usbh_urb_enqueue(port_instance->dflt_pipe_handle, urb_hdl, xfer_size);
    ERR_CHECK(ESP_OK == ret, "urb enqueue failed", NULL);
    ret = esp_usbh_pipe_event_wait(port_instance->dflt_pipe_handle, PIPE_EVENT_URB_DONE, pdMS_TO_TICKS(TIMEOUT_USBH_CTRL_XFER_MS));
    ERR_CHECK(ESP_OK == ret, "urb event error", NULL);
    esp_usbh_urb_handle_t urb_done = esp_usbh_urb_dequeue(port_instance->dflt_pipe_handle, xfered_size, status);
    return urb_done;
}
...
static esp_err_t _usbh_get_dev_desc(usbh_pipe_handle_t pipe_handle, usb_device_desc_t *device_desc)
{
    ...
    urb_t *urb_ctrl = esp_usbh_urb_alloc(0, sizeof(usb_setup_packet_t) + CTRL_TRANSFER_DATA_MAX_BYTES, NULL);
    ...
    esp_err_t ret = esp_usbh_urb_enqueue(pipe_handle, urb_ctrl, num_bytes);
    ERR_CHECK_GOTO(ESP_OK == ret, "urb enqueue failed", free_urb_);

    // waiting for transmit done
    ret = esp_usbh_pipe_event_wait(pipe_handle, PIPE_EVENT_URB_DONE, pdMS_TO_TICKS(TIMEOUT_USBH_CTRL_XFER_MS));
    ERR_CHECK_GOTO(ESP_OK == ret, "urb event error", flush_urb_);

    // dequeue URB, check transmit result
    urb_done = esp_usbh_urb_dequeue(pipe_handle, &num_bytes, &status);
...
flush_urb_:
    esp_usbh_pipe_flush(pipe_handle, 1);
free_urb_:
    esp_usbh_urb_free(urb_ctrl);
    return ret;
}
#endif
...
static esp_err_t _usbh_get_config_desc(usbh_pipe_handle_t pipe_handle, usb_config_desc_t **config_desc)
{
    ...
    ret = esp_usbh_urb_enqueue(pipe_handle, urb_ctrl, num_bytes);
    ERR_CHECK_GOTO(ESP_OK == ret, "urb enqueue failed", free_urb_);
    ret = esp_usbh_pipe_event_wait(pipe_handle, PIPE_EVENT_URB_DONE, pdMS_TO_TICKS(TIMEOUT_USBH_CTRL_XFER_MS));
    ERR_CHECK_GOTO(ESP_OK == ret, "urb event error", flush_urb_);
    urb_done = esp_usbh_urb_dequeue(pipe_handle, &num_bytes, &status);
...
static esp_err_t _usbh_set_device_addr(usbh_pipe_handle_t pipe_handle, uint8_t dev_addr)
{
    ...
    //malloc URB for default control
    urb_t *urb_ctrl = esp_usbh_urb_alloc(0, sizeof(usb_setup_packet_t), NULL);
    ERR_CHECK(urb_ctrl != NULL, "alloc urb failed", ESP_ERR_NO_MEM);
    ...
    // dequeue URB, check transmit result
    urb_t *urb_done = esp_usbh_urb_dequeue(pipe_handle, &num_bytes, &status);
    ERR_CHECK_GOTO(urb_done == urb_ctrl, "urb status: not same", free_urb_);
    ERR_CHECK_GOTO(USB_TRANSFER_STATUS_COMPLETED == status, "urb status: not complete", free_urb_);
            ...
            case PORT_EVENT:
                ...
                case PORT_EVENT_CONNECTION:
                    ...
                    if (!pipe_instance_dflt) {
                        pipe_instance_dflt = esp_usbh_pipe_init(port_instance, NULL, NULL, NULL);
                    }
...
usb_driver_reset_:
        ...
        if (pipe_instance_dflt != NULL) {
            ...
            esp_usbh_pipe_flush(pipe_instance_dflt, 1);
            esp_usbh_pipe_deinit(pipe_instance_dflt);
