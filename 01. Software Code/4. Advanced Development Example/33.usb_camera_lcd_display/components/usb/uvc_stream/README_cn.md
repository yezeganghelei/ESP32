...
Note: The driver has been migrated to [master/components/usb/usb_stream](https://github.com/espressif/esp-iot-solution/tree/master/components/usb/usb_stream), the new version supports UVC + UAC function
...
* USB Camera
  ...
  3. 如use同步传输Camera，Need to support setting interface `Max Packet Size` for `512`
  4. Isochronous transfer mode image data stream USB The total transmission bandwidth should be less than `4 Mbps` （500 KB/s）
  5. Batch transfer mode image data stream USB The total transmission bandwidth should be less than `8.8 Mbps` （1100 KB/s）
  6. Please refer to the sample program description file for details on resolution and other requirements.

### UVC Stream API use说明

1. 用户需提前了解待适配Camera`configuration descriptor`Detailed parameters of，Fill in accordingly `uvc_config_t` configuration items，The corresponding relationship between parameters is as follows：

```
    uvc_config_t uvc_config = {
        .xfer_type = UVC_XFER_ISOC,  //设置Camera模式for同步传输
