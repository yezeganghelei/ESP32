# 定制Model的step介绍 [[English]](./README.md)

本教程介绍了定制Model的详细step。The examples in the tutorial are runnable [MNIST](https://tensorflow.google.cn/datasets/catalog/mnist) Classification items，Hereinafter referred to as MNIST。

relatedlike何定制layer，Please check[定制layer的step介绍](../docs/zh_CN/implement_custom_layer.md)。
...
use [numpy.save ()](https://numpy.org/doc/stable/reference/generated/numpy.save.html?highlight=save#numpy.save) function，save .npy model floating point coefficients in format：

```
numpy.save(file=f'{filename}', arr=coefficient)
```

Each layer of the neural network needs to have：

- **Filter**: Save as `'{layer_name}_filter.npy``
- **deviation**：save as `'{layer_name}_bias.npy'`
- **激活function**：activation function with coefficients，like *LeakyReLU*、*PReLU*，save as `'{layer_name}_activation.npy'`

**Example**： [`./model/npy/`](./model/npy/) document夹中 .npy document里的 MNIST item coefficient。

## step 2：Configure model

according to [**config.json Configuration specifications**](../tools/convert_tool/specification_of_config_json_cn.md)，exist config.json Configure model in file。

**Example**： [`./model/npy/config.json`](./model/npy/config.json) document中 MNIST Project configuration。

## step 3：Convert model coefficients

Will coefficient.npy documentand config.json Once ready and saved in the same folder，use convert.py（Please refer to [**convert.py use说明**](../tools/convert_tool/README_cn.md)）Convert the coefficients to C/C++ code。
...
Then [`./model/`](./model/) document夹中会生成两个document：`mnist_coefficient.cpp` and `mnist_coefficient.hpp`。

after，call `get_{layer_name}_***()` 即可获取每layer的系数。比like要获取 "l1" filter，可call `get_l1_filter()`。

## step 4：Build model

### step 4.1：from [`"dl_layer_model.hpp"`](../include/layer/dl_layer_model.hpp) Derive a new class from the model class in
...
### step 4.3：用构造function初始化layer

[step 3：Convert model coefficients](#step-3Convert model coefficients)generated `"mnist_coefficient.hpp"` There are coefficients of layers in the file，用该系数初始化layer。

relatedlike何初始化每一layer，Please check [esp-dl/include/layer/](../include/layer/) document夹中相应的 .hpp document。
...
### step 4.4：accomplish `void build(Tensor<input_t> &input)`

In order to facilitate the distinction`Model` `build()` and`layer` `build()`，current definition：

* `Model` `build()` is `Model.build()`;
* `layer` `build()` for `Layer.build()`。

`Model.build()` 会call所有 `Layer.build()`。`Model.build()` 仅exist输入形状变化时有效。If the input shape does not change，but `Model.build()` 不会被call，thus saving calculation time。

related `Model.build()` 何时被call，Please check[step 5：runModel](#step-5runModel)。

relatedlike何call每一layer的 `Layer.build()`，Please check [esp-dl/include/layer/](../include/layer/) document夹中相应的 .hpp document。
...
`Model.call()` 会call所有 `Layer.call()`。relatedlike何call每一layer的 `Layer.call()`，Please check [esp-dl/include/layer/](../include/layer/) document夹中相应的 .hpp document。
...
- run `Model.forward()` Perform neural network inference。`Model.forward()` The process is as follows：
...
**Example**：[`./main/main.cpp`](./main/app_main.cpp) in the file MNIST object and `forward()` function。
