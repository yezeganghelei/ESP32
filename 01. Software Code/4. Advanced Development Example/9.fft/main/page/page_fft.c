 ...
 * @LastEditTime: 2023-07-17 15:20:06
 * @FilePath: \SP_V2_DEMO\9.fft\main\page\page_fft.c
 */
#include "page_fft.h"
#include "app_main.h"
#include "stdio.h"
...
#define ANIEND  while(lv_anim_count_running())lv_task_handler();//Wait for animation to complete
...
static void Title_Create()
{
	...
	lv_style_set_bg_color(&style_cont, LV_STATE_DEFAULT, LV_COLOR_BLACK); //Set screen background
...
static void Cont_create(void)
{
	...
	lv_chart_set_type(chart_fft, LV_CHART_TYPE_COLUMN); //straight column pattern

	// lv_chart_set_type(chart_fft, LV_CHART_TYPE_LINE); //Polyline mode

	lv_chart_set_range(chart_fft, 0, APP_WIN_HEIGHT);

	series_fft = lv_chart_add_series(chart_fft, LV_COLOR_RED);
}
// //When the current interface is a menuMOVEtask to do
