...
lv_obj_t *img_cam; //to display image
...
void Cam_Task(void *pvParameters)
{
	...
	while (1)
	{
		...
		fb = esp_camera_fb_get();//Get the image
		...
		{
			img_dsc.data = fb->buf;//Pass the obtained image data toimg_dsc
			lv_img_set_src(img_cam, &img_dsc);//set upimg_camThe source file isimg_dsc  Complete display picture
			// Decode Progress
			esp_image_scanner_t *esp_scn = esp_code_scanner_create();
			...
			if (decoded_num)//If a QR code is recognized
			{
				...
				lv_label_set_text(label_detect, "Nothing..."); //not recognized
...
void page_cam_load()
{
	...
	vTaskDelay(100);
	imgcam_init();//initialize aimgcamfor display
	vTaskDelay(1000);
