...
{
    return generic_command_default_handle(dce, "AT\r\n");
}
...
esp_err_t esp_modem_dce_set_echo(esp_modem_dce_t *dce, void *param, void *result)
{
    ...
    if (echo_on) {
        return generic_command_default_handle(dce, "ATE1\r\n");
    } else {
        return generic_command_default_handle(dce, "ATE0\r\n");
    }
}
...
{
    common_string_t common_str = { .command = "AT+CGSN?\r\n", .string = result, .len = (size_t)param };
...
{
    return esp_modem_dce_generic_command(dce,  "AT+CRESET\r\n", MODEM_COMMAND_TIMEOUT_RESET, esp_modem_dce_handle_reset, NULL);
}
...
{
    return esp_modem_dce_generic_command(dce,  "AT+CPIN?\r\n", MODEM_COMMAND_TIMEOUT_DEFAULT, esp_modem_dce_handle_read_pin, result);
}

esp_err_t esp_modem_dce_store_profile(esp_modem_dce_t *dce, void *param, void *result)
{
    return generic_command_default_handle(dce, "AT&W\r\n");
}
...
esp_err_t esp_modem_dce_set_flow_ctrl(esp_modem_dce_t *dce, void *param, void *result)
{
    ...
    char *command;
    int len = asprintf(&command, "AT+IFC=%d,%d\r\n", dte->flow_ctrl, flow_ctrl);
    if (len <= 0) {
...
esp_err_t esp_modem_dce_set_pdp_context(esp_modem_dce_t *dce, void *param, void *result)
{
    ...
    char *command;
    int len = asprintf(&command, "AT+CGDCONT=%d,\"%s\",\"%s\"\r\n", pdp->cid, pdp->type, pdp->apn);
    if (len <= 0) {
        return ESP_ERR_NO_MEM;
    }
    esp_err_t err = generic_command_default_handle(dce, command);
...
{
    return esp_modem_dce_generic_command(dce, "ATH\r\n", MODEM_COMMAND_TIMEOUT_HANG_UP,
...
{
    return esp_modem_dce_generic_command(dce, "AT+CSQ\r\n", MODEM_COMMAND_TIMEOUT_DEFAULT,
...
{
    return esp_modem_dce_generic_command(dce, "AT+QPOWD=1\r\n", MODEM_COMMAND_TIMEOUT_POWEROFF,
                                         esp_modem_dce_handle_power_down, NULL);
}

esp_err_t esp_modem_dce_set_baud_temp(esp_modem_dce_t *dce, void *param, void *result)
{
    char command[] = "AT+IPR=3686400\r\n"; // reserve space with max baud placeholder
