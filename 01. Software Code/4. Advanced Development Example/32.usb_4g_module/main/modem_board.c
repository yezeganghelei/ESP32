...
{
    ESP_LOGW(TAG, "Force reset modem");
    ...
    gpio_set_level(MODEM_RESET_GPIO, !MODEM_RESET_GPIO_INACTIVE_LEVEL);
    ESP_LOGW(TAG, "Resetting modem using io=%d, level=%d", MODEM_RESET_GPIO, !MODEM_RESET_GPIO_INACTIVE_LEVEL);
    vTaskDelay(pdMS_TO_TICKS(MODEM_RESET_GPIO_ACTIVE_MS));
    gpio_set_level(MODEM_RESET_GPIO, MODEM_RESET_GPIO_INACTIVE_LEVEL);
    // waitting for modem re-init ready
    ESP_LOGW(TAG, "Waiting for modem initialize ready");
...
static esp_err_t my_recov(esp_modem_recov_resend_t *retry_cmd, esp_err_t err, int timeouts, int errors)
{
    ...
    if (err != ESP_OK) {
        if (timeouts + errors < 2) {
            ...
            if (s_led_system_handle) {
                led_indicator_start(s_led_system_handle, BLINK_FACTORY_RESET);
            }
            modem_board_t *board = __containerof(dce, modem_board_t, parent);
            board->reset(dce);
            if (s_led_system_handle) {
                led_indicator_stop(s_led_system_handle, BLINK_FACTORY_RESET);
            }
        } else {
...
static esp_modem_dce_t *modem_board_create(esp_modem_dce_config_t *config)
{
    ...
    ESP_MODEM_EXAMPLE_CHECK(board, "failed to allocate modem_board object", err);
    ESP_MODEM_EXAMPLE_CHECK(esp_modem_dce_init(&board->parent, config) == ESP_OK, "Failed to init modem_dce", err);
...
static void _usb_dte_conn_callback(void *arg)
{
    ...
    if (s_led_system_handle) {
        led_indicator_stop(s_led_system_handle, BLINK_RECONNECTING);
    }
    return;
...
static void _usb_dte_disconn_callback(void *arg)
{
    ...
    if (s_led_system_handle) {
        led_indicator_start(s_led_system_handle, BLINK_RECONNECTING);
    }
}
...
esp_err_t modem_board_init(modem_config_t *config)
{
    ...
    if (LED_RED_SYSTEM_GPIO) {
        s_led_system_handle = led_indicator_create(LED_RED_SYSTEM_GPIO, &led_config);
        assert(s_led_system_handle != NULL);
    }
    if (LED_BLUE_WIFI_GPIO) {
        s_led_wifi_handle = led_indicator_create(LED_BLUE_WIFI_GPIO, &led_config);
        assert(s_led_wifi_handle != NULL);
        led_indicator_stop(s_led_wifi_handle, BLINK_CONNECTED);
        led_indicator_start(s_led_wifi_handle, BLINK_CONNECTING);
    }
    if (LED_GREEN_4GMODEM_GPIO) {
        s_led_4g_handle = led_indicator_create(LED_GREEN_4GMODEM_GPIO, &led_config);
...
{
    if (dns == NULL || type > ESP_NETIF_DNS_BACKUP) {
