...
const uint8_t * lv_font_get_bitmap_fmt_txt(const lv_font_t * font, uint32_t unicode_letter)
{
    ...
    lv_font_fmt_txt_dsc_t * fdsc = (lv_font_fmt_txt_dsc_t *) font->dsc;//Get font source
    uint32_t gid = get_glyph_dsc_id(font, unicode_letter);//gid is the number of characters
    if(!gid) return NULL;

    const lv_font_fmt_txt_glyph_dsc_t * gdsc = &fdsc->glyph_dsc[gid];//Get gid word information

    if(fdsc->bitmap_format == LV_FONT_FMT_TXT_PLAIN) {//Is there any compression?
...
static uint32_t get_glyph_dsc_id(const lv_font_t * font, uint32_t letter)
{
    ...
    uint16_t i;
    for(i = 0; i < fdsc->cmap_num; i++) {//Numbers and Chinese characters

        /*Relative code point*/
        uint32_t rcp = letter - fdsc->cmaps[i].range_start;
        if(rcp > fdsc->cmaps[i].range_length) continue;
        uint32_t glyph_id = 0;
        if(fdsc->cmaps[i].type == LV_FONT_FMT_TXT_CMAP_FORMAT0_TINY) {//If it is the number 3, rcp=51-49=2
        ...
        else if(fdsc->cmaps[i].type == LV_FONT_FMT_TXT_CMAP_SPARSE_TINY) {//If it is the Chinese character - king, rcp=29579-97=29482
            uint16_t key = rcp;
