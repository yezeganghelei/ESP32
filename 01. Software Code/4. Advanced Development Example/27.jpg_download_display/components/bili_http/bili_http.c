...
void get_face_jpg(char *url_jpg)
{
	printf("%s !Dram: %d bytes\r\n", __func__, heap_caps_get_free_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT));
	...
	// if (err == ESP_OK) //Request successful
	// {
	...
	else
	{
		...
		if (root_ksdiy != NULL)
		{
			...
			if (psub_ksdiy != NULL)
			{
				...
				ESP_LOGI(TAG, "Successfully obtained avatar URL name:%s face_url:%s\n", wp_fans.name, wp_fans.face_url);
