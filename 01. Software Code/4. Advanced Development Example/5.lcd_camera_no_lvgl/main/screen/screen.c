					 ...
					 int16_t GPIO_MISO, int16_t XPT_CS, int16_t XPT_IRQ)
{
	...
	spi_bus_config_t buscfg = {
		...
		.mosi_io_num = GPIO_MOSI,
		.miso_io_num = -1,
		.max_transfer_sz = 4095,                    // Maximum transfer data size
		.quadwp_io_num = -1,
		.quadhd_io_num = -1};
#endif
...
void lcdInit(TFT_t *dev, uint16_t model, int width, int height, int offsetx, int offsety)
{
	...
	if (dev->_model == 0x7796)
	{
		...
		spi_master_write_data_byte(dev, 0x00);	//Right top start, BGR color filter panel
	...
	if (dev->_model == 0x9340 || dev->_model == 0x9341 || dev->_model == 0x7735)
	{
		...
		spi_master_write_comm_byte(dev, 0xB1); // Frame Rate Control
		spi_master_write_data_byte(dev, 0x00);
		spi_master_write_data_byte(dev, 0x58);
...
void lcdDrawMultiPixels(TFT_t *dev, uint16_t x, uint16_t y, uint16_t size, uint16_t *colors)
{
	...
	// ESP_LOGI(TAG, "offset(x)=%d offset(y)=%d", dev->_offsetx, dev->_offsety);
	uint16_t _x1 = x + dev->_offsetx;
	uint16_t _x2 = _x1 + size;
	uint16_t _y1 = y + dev->_offsety;
	uint16_t _y2 = _y1;
	// ESP_LOGI(TAG, "_x1=%d _x2=%d _y1=%d _y2=%d", _x1, _x2, _y1, _y2);
