...
/*
 フォントパターンをビットマップイメージに変change する
...
#if 0
// UTF code(3Byte) を SJIS Code(2 Byte) に変change
...
uint16_t UTF2SJIS(spiffs_file fd, uint8_t *utf8) {
  ...
  if(utf8[0]>=0xC2 && utf8[0]<=0xD1){ //0xB0からS_JISコード実データ. 0x00-0xAFまではライセンスartsヘッダなのでそれをカット.
    offset = ((utf8[0]*256 + utf8[1])-0xC2A2)*2 + 0xB0; //Word"￠" UTF8コード C2A2～、S_jisコード8191
  }else if(utf8[0]==0xE2 && utf8[1]>=0x80){
    offset = (UTF8uint-0xE28090)*2 + 0x1EEC; //Text "‐" UTF8コード E28090～、S_jisコード815D
    ...
    offset = (UTF8uint-0xE4B880)*2 + 0x11CCC; //Word"one" UTF8コード E4B880～、S_jisコード88EA
  }else if(utf8[0]==0xE5 && utf8[1]>=0x80){
    offset = (UTF8uint-0xE58085)*2 + 0x12BCC; //Word"second" UTF8コード E58085～、S_jisコード98E4
  }else if(utf8[0]==0xE6 && utf8[1]>=0x80){
    offset = (UTF8uint-0xE6808E)*2 + 0x1AAC2; //Text "How" UTF8コード E6808E～、S_jisコード9C83
  }else if(utf8[0]==0xE7 && utf8[1]>=0x80){
    offset = (UTF8uint-0xE78081)*2 + 0x229A6; //Word"overflow" UTF8コード E78081～、S_jisコードE066
  }else if(utf8[0]==0xE8 && utf8[1]>=0x80){
    offset = (UTF8uint-0xE88080)*2 + 0x2A8A4; //Word"dazzle" UTF8コード E88080～、S_jisコード9773
  }else if(utf8[0]==0xE9 && utf8[1]>=0x80){
    offset = (UTF8uint-0xE98080)*2 + 0x327A4; //Word"retreat" UTF8コード E98080～、S_jisコード91DE
  }else if(utf8[0]>=0xEF && utf8[1]>=0xBC){
    offset = (UTF8uint-0xEFBC81)*2 + 0x3A6A4; //Word"！" UTF8コード EFBC81～、S_jisコード8149
...
// UTFをContainsむWord列をSJISにChange
...
if(FontxDebug)printf("[String2SJIS]sp[%d]=%x\n",i,sp);
    if ((sp & 0xf0) == 0xe0) { // Superior position4ビットが1110なら、3バイトWordの1バイトHead
      c1 = sp;
    } else if ((sp & 0xc0) == 0x80) { // Superior position2ビットが10なら、heバイトWordの2バイトHead以降
      if (c2 == 0) {
        c2 = sp;
      } else {
        if (c1 == 0xef && c2 == 0xbd) {
...
if(FontxDebug)printf("[String2SJIS]sjis2=%x\n",sjis2);
    ...
    } else if ((sp & 0x80) == 0) { // 1バイトWordのoccasion
