...
/*
 * Speech recognition processing function
 */
static esp_err_t asr_multinet_control(int commit_id)
{
    ...
    if (commit_id >= 0 && commit_id < ID_MAX)
    {
        ...
        case ID15_DAKAIKONGTIAO:
            ...
            lv_label_set_text(label_speech, "Air conditioner turned on");
        ...
        case ID16_GUANBIKONGTIAO:
            ...
            lv_label_set_text(label_speech, "Air conditioner turned off");
        ...
        case ID17_GELIKONGTIAO:
            ...
            lv_label_set_text(label_speech, "Already set as Gree air conditioner");
            break;
        case ID18_MEIDIKONGTIAO:
            play_spiffs_name("haode.wav");

            ESP_LOGI(TAG, "ID18_MEIDIKONGTIAO");
            lv_label_set_text(label_speech, "Already set to Midea air conditioner");
            break;
        case ID19_AOKESIKONGTIAO:
            play_spiffs_name("haode.wav");

            ESP_LOGI(TAG, "ID19_AOKESIKONGTIAO");
            lv_label_set_text(label_speech, "Already set to Oaks air conditioner");
            break;
        case ID20_HAIERKONGTIAO:
            play_spiffs_name("haode.wav");

            ESP_LOGI(TAG, "ID20_HAIERKONGTIAO");
            lv_label_set_text(label_speech, "Already set as Haier air conditioner");
            break;
        case ID21_KAIDENG:
            play_spiffs_name("haode.wav");
            app_led_set_all(50, 50, 50);
            ESP_LOGI(TAG, "ID21_KAIDENG");
            lv_label_set_text(label_speech, "Lights on");
            break;
        case ID22_GUANDENG:
            play_spiffs_name("haode.wav");
            app_led_set_all(0, 0, 0);
            ESP_LOGI(TAG, "ID22_GUANDENG");
            lv_label_set_text(label_speech, "Lights off");
            break;
        case ID23_HONGSE:
            play_spiffs_name("haode.wav");
            app_led_set_all(100, 0, 0);
            ESP_LOGI(TAG, "ID23_HONGSE");
            lv_label_set_text(label_speech, "OK");
            break;
        case ID24_LANSE:
            play_spiffs_name("haode.wav");
            app_led_set_all(0, 0, 100);
            ESP_LOGI(TAG, "ID24_LANSE");
            lv_label_set_text(label_speech, "OK");
            break;
        case ID25_LVSE:
            play_spiffs_name("haode.wav");
            app_led_set_all(0, 100, 0);
            ESP_LOGI(TAG, "ID25_LVSE");
            lv_label_set_text(label_speech, "OK");
            break;
        default:
    ...
    while (1)
    {
        ...
        if (detect_flag)
        {
            ...
            else if (command == COMMAND_TIMEOUT)
            {
                ...
                vTaskDelay(100);
                lv_label_set_text(label_speech, "Didn't hear clearly just now...");
...
void app_speech_wakeup_init()
{
    ...
    xTaskCreatePinnedToCore(&recsrcTask, "recsrcTask", 4 * 1024, (void *)afe_data, 13, NULL, 0); // Create a recording task

    xTaskCreatePinnedToCore(&nnTask, "nnTask", 5 * 1024, (void *)afe_data, 14, NULL, 1); // Create recognition tasks
}