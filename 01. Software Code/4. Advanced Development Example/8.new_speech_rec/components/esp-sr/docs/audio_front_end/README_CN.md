...
用户use `afe_handle->create_from_config(&afe_config)` 函数来获得data句柄，这将会existafeinternaluse，The parameter passed in is the above2步中获得configuration。
...
The input audio size and layout format can be referred to **input audio** this step。
