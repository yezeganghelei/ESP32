...
at present，Users can use `make menuconfig` command to add custom command words。可by通过 `menuconfig -> ESP Speech Recognition->Add speech commands` Add command word，Already added 20 indivualChinesecommand word和 7 indivualEnglishcommand word，As shown in the table below：
