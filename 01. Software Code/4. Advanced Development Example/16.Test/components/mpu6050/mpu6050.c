...
{
    return (mpu6050_get_device_id() == 0x34 || mpu6050_get_device_id() == 0x0c);
}
...
void IMU_update(float dT, _xyz_f_st *gyr, _xyz_f_st *acc, _imu_st *imu)
{
	...
	// accelerometer reading，Unitization。
	...
	// under the carrier coordinatesxdirection vector，Unitization。
	imu->x_vec.x = 1 - (2 * q2q2 + 2 * q3q3);
	imu->x_vec.y = 2 * q1q2 - 2 * q0q3;
	imu->x_vec.z = 2 * q1q3 + 2 * q0q2;

	// under the carrier coordinatesydirection vector，Unitization。
	imu->y_vec.x = 2 * q1q2 + 2 * q0q3;
	imu->y_vec.y = 1 - (2 * q1q1 + 2 * q3q3);
	imu->y_vec.z = 2 * q2q3 - 2 * q0q1;

	// under the carrier coordinateszdirection vector（Equivalent gravity vector、Gravity acceleration vector），Unitization。
	imu->z_vec.x = 2 * q1q3 - 2 * q0q2;
	imu->z_vec.y = 2 * q2q3 + 2 * q0q1;
	imu->z_vec.z = 1 - (2 * q1q1 + 2 * q2q2);

	// Calculate motion acceleration in carrier coordinates。(Has nothing to do with attitude calculation)
	imu->a_acc.x = acc->x - 9800 * imu->z_vec.x;
	imu->a_acc.y = acc->y - 9800 * imu->z_vec.y;
	imu->a_acc.z = acc->z - 9800 * imu->z_vec.z;

	// Calculate the motion acceleration in world coordinates. (It has nothing to do with attitude calculation)
	...
	// The cross product of the measured value and the equivalent gravity vector (computing the vector error).
	vec_err.x = (acc_norm.y * imu->z_vec.z - imu->z_vec.y * acc_norm.z);
	vec_err.y = -(acc_norm.x * imu->z_vec.z - imu->z_vec.x * acc_norm.z);
	vec_err.z = 0;

	//Low-pass limiting filter with cutoff frequency 1hz
	...
	// Construct incremental rotation（Contains fusion correction）。
