...
`lv_ime_pinyin`for[keyboard](/widgets/extra/keyboard)The component provides Chinese Pinyin input method（Chinese输入）function(Hereinafter referred to as Pinyin input method)，support26key and9Key input mode。您可以Will `lv_ime_pinyin` Chinese Pinyin input method plug-in regarded as a keyboard component。
 
Normally，As long as it is[keyboard](/widgets/extra/keyboard)组件能运行of环境 `lv_ime_pinyin` Also works。There are two influencing factors：Font size and vocabulary size。
...
After writing your own lexicon according to the above lexicon format,，Refer to the usage below，call `lv_100ask_pinyin_ime_set_dict(pinyin_ime, your_pinyin_dict)` 函数That’s it设置和use新词库：
...
`lv_ime_pinyin` support26key and9Key input mode。Switching modes is very easy，只需call函数 `lv_ime_pinyin_set_mode` That’s it。if function `lv_ime_pinyin_set_mode` of the2parameters are `1` then switch to 26 Key input mode，如果for `0` then switch to 9 key input method mode，Default is `1` 。
