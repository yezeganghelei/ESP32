...
const uint8_t * lv_font_get_bitmap_fmt_txt(const lv_font_t * font, uint32_t unicode_letter)
{
    ...
    uint32_t gid = get_glyph_dsc_id(font, unicode_letter);//gidFor the first few words
    if(!gid) return NULL;
...
static uint32_t get_glyph_dsc_id(const lv_font_t * font, uint32_t letter)
{
    ...
    for(i = 0; i < fdsc->cmap_num; i++) {//Numbers and Chinese characters
        ...
        else if(fdsc->cmaps[i].type == LV_FONT_FMT_TXT_CMAP_SPARSE_TINY) {//If it is Chinese characters - Wang, rcp=29579-97=29482
            uint16_t key = rcp;
