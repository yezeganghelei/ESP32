...
#define USE_LCD_BUF    1            /* Used by defaultlcd_buf,uselcd_bufThe refresh speed will be greatly improved,Sacrifice space to increase speed,When memory is insufficient,set0Just */
#if USE_LCD_BUF
    #if SPI_LCD_TYPE
...
void lcd_display_dir(uint8_t dir)
{
    ...
    if (SPI_LCD_TYPE)   /* 2.4inchThe screen needs processing，1.3The screen does not need to be processed */
    {
...
void lcd_init(void)
{
    ...
    lcd_write_cmd(0x29);
#endif
    LCD_CS(1);                  /* High-high film selection,Uncheck */  
...
void lcd_show_char(uint16_t x, uint16_t y, char ch, lcd_font_t font, uint8_t mode, uint16_t color)
{
    ...
    for (byte_index = 0; byte_index < ch_size; byte_index++)
    {
        ...
        for (bit_index = 0; bit_index < 8; bit_index++)   /* 8 points per byte */
        {
            if ((byte_code & 0x80) != 0)                  /* Valid points, need to be displayed */
            {
                lcd_draw_point(x + width_index, y + height_index, color);           /* Draw pointscome out,To display this point */
            }
            else if (mode == 0)
            {
                lcd_draw_point(x + width_index, y + height_index, g_back_color);    /* paintingbackground color,Equivalent to thisOne dotNot displayed(Note that the background color is controlled by global variables) */
            }
 ...
 *              mode : NUM_SHOW_NOZERO: Number high0Not displayed
 ...
 * @note        Picture taking mode: horizontal scan、RGB565、High position first
