...
/**
* @brief       initializationIOExpand chip
* @param       none
* @retval      none
*/
...
void xl9555_io_config(uint16_t port_pin, io_mode_t mode)
{
    ...
    {
        config_value = config_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));   /* getSomeIOset upAfter the output functionofPORTvalue but does not affect theset upofotherIOThe status of */
    }
    else
    {
        config_value = config_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));      /* getSomeIOset as input functionPORTvalue but does not affect theset upofotherIOThe status of */
    }
...
void xl9555_pin_set(uint16_t port_pin, io_state_t state)
{
    ...
    {
        pin_value = pin_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));          /* getSomeIOAfter setting to high levelPORTvalue but does not affect theset upofotherIOThe status of */
    }
    else
    {
        pin_value = pin_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));       /* getSomeIOset upforlow levelbackofPORTvalue but does not affect theset upofotherIOThe status of */
    }
