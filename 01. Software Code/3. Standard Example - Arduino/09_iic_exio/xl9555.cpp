...
void xl9555_io_config(uint16_t port_pin, io_mode_t mode)
{
    ...
    {
        config_value = config_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));   /* getsomeoneIOset upAfter the output functionofPORTvalue but does not affect theset upofotherIOstatus */
    }
    else
    {
        config_value = config_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));      /* Get someoneIOSet as input functionPORT value but does not affect other IOstatus that are not set upof */
    }
...
void xl9555_pin_set(uint16_t port_pin, io_state_t state)
{
    ...
    {
        pin_value = pin_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));          /* getsomeoneIOAfter setting to high levelPORTvalue but does not affect theset upofotherIOstatus */
    }
    else
    {
        pin_value = pin_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));       /* getsomeoneIOset upforLow levelbackofPORTvalue but does not affect theset upofotherIOstatus */
    }
