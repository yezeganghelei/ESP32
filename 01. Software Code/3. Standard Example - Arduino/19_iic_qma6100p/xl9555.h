...
#define XL9555_INVERSION_PORT0_REG    4           /* Polarity inversion register: Used whenP0 When the port is used as input, Reversal of the input level, That is, when the pin is high level, set up the corresponding of bit in this register is 1hour, read to ofInput Register0, 1The value is low level0 */
#define XL9555_INVERSION_PORT1_REG    5           /* Polarity inversion register：Used whenP1When the port is used as input，Reversal of the input level，That is, when the pin is high level，set upThis register corresponds toofThe bit is1hour，readarriveofInput Register0，1The value is low level0 */
...
uint8_t xl9555_get_pin(uint16_t port_pin);                  /* GetXL9555Configured as input functionIOThe status of(High or low) */
