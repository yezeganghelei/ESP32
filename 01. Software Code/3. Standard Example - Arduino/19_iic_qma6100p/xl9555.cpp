...
/**
 * @brief       TowardsXL9555Related registers read data
 * @param       reg    : Register address
...
/**
 * @brief set upXL9555SomeIOThe mode(output or input)
...
void xl9555_io_config(uint16_t port_pin, io_mode_t mode)
{
    ...
    config_reg  = xl9555_read_reg(port_pin > XL_PORT0_ALL_PIN ? XL9555_CONFIG_PORT1_REG : XL9555_CONFIG_PORT0_REG);   /* First Readset upPinWhereof register situation */

    if (mode == IO_SET_OUTPUT)    /* According to modeparameter set up input and output conditions, cannot affect otherIO */
    {
        config_value = config_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));   /* have toarriveSomeIOset upforOutput functionbackofPORTvalue but does not affect theset upofotherIOstatus */
    }
    else
    {
        config_value = config_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));      /* arriveSomeIOSet as input functionPORTvalue but does not affect theset upofotherIOstatus */
    }
...
void xl9555_pin_set(uint16_t port_pin, io_state_t state)
{
    ...
    pin_reg = xl9555_read_reg(port_pin > XL_PORT0_ALL_PIN ? XL9555_OUTPUT_PORT1_REG : XL9555_OUTPUT_PORT0_REG);     /* First Readset upPinWhereof register situation */

    if (state == IO_SET_HIGH)    /* according to stateparameter set upIOof high low level */
    {
        pin_value = pin_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));          /* have toarriveSomeIOAfter setting to high levelPORTvalue but does not affect theset upofotherIOstatus */
    }
    else
    {
        pin_value = pin_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));       /* have toarriveSomeIOset upforlow levelbackofPORTvalue but does not affect theset upofotherIOstatus */
    }
