...
void xl9555_init(void)
{
    ...
    /* After power-on, read once to clear the interrupt flag. */
    xl9555_read_port(0);
    xl9555_read_port(1);
}
...
void xl9555_write_reg(uint8_t reg, uint8_t data)
{
    ...
    Wire.endTransmission();                   /* IIC send Send the queue data(Withoutparameter,expresssendstopSignal,End transmission) */          
}
...
uint8_t xl9555_read_reg(uint8_t reg)
{
    ...
    Wire.endTransmission(0);                  /* IIC send Send the queue data(Pass parameters as0,Indicates resending onestartSignal,KeepIICbus validconnect) */
 ...
 * @param       port_pin  : wantset upofIOserial number,P0~7orP1~7
...
void xl9555_io_config(uint16_t port_pin, io_mode_t mode)
{
    ...
    config_reg  = xl9555_read_reg(port_pin > XL_PORT0_ALL_PIN ? XL9555_CONFIG_PORT1_REG : XL9555_CONFIG_PORT0_REG);   /* Firstreadset upPinlocationofRegister status */
    ...
    xl9555_write_reg(port_pin > XL_PORT0_ALL_PIN ? XL9555_CONFIG_PORT1_REG : XL9555_CONFIG_PORT0_REG, config_value);    /* Set the IO input and output status to the configuration register */
}
...
void xl9555_pin_set(uint16_t port_pin, io_state_t state)
{
    ...
    pin_reg = xl9555_read_reg(port_pin > XL_PORT0_ALL_PIN ? XL9555_OUTPUT_PORT1_REG : XL9555_OUTPUT_PORT0_REG);     /* Firstreadset upPinlocationofRegister status */
    ...
    {
        pin_value = pin_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));       /* getarrivesomeoneIOset upforlow levelbackofPORTvalue but does not affect theset upofotherIOstatus */
    }
