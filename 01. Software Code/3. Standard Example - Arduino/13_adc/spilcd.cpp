...
#define USE_LCD_BUF    1            /* Used by defaultlcd_buf,uselcd_bufThe refresh speed will be greatly improved,Sacrifice space for speed,When memory is insufficient,set0That’s it */
#if USE_LCD_BUF
    #if SPI_LCD_TYPE
    uint16_t lcd_buf[320 * 240];    /* Store a frame of imagedata */
    #else
    uint16_t lcd_buf[240 * 240];    /* Store a frame of imagedata */
    #endif
#endif
 ...
 * @param       data:dataofstarting address
 ...
 * @note        generallyset upforL2R_U2DThat’s it,ifset upFor other scanning methods,May cause abnormal display.
 * @param       dir:0~7,represent8direction(See the specific definition forlcd.h)
 * @retval      none
 */
void lcd_scan_dir(uint8_t dir)
{
    ...
    if (spilcd_dir == 1)  /* When screening horizontally，Need to change scanning direction！ */
    {
    ...
    if (regval & 0x20)
    {
        if (spilcd_width < spilcd_height)   /* Swap X,Y */
        {
            temp = spilcd_width;
...
void lcd_display_dir(uint8_t dir)
{
    ...
    if (SPI_LCD_TYPE)   /* 2.4inchThe screen needs processing，1.3No processing required in-screen */
    {
...
/**
 * @brief       Clear the screenfunction
 * @param       color: wantClear the screenofcolor
 * @retval      none
 */
...
void lcd_init(void)
{
    ...
    /* LCDThe pins required for the screenDefault stateforhigh level */
    ...
    spi_lcd = new SPIClass(HSPI);   /* createSPIClassSelect the instanceSPIbus(HSPI) */
    ...
    LCD_CS(0);                  /* Pull chip select line low,Select the device */
    ...
    lcd_display_on();           /* OpenLCDbacklight */
    lcd_clear(WHITE);           /* Clear the screen */
}

/**
 * @brief       OpenLCDbacklight
 * @param       none
 * @retval      none
 */
 ...
 *              xe   : zone terminationXcoordinate
 *              ye   : zone terminationYcoordinate
...
/**
 * @brief       LCDDraw points
 * @param       x    : treatDraw pointsofXcoordinate
 *              y    : treatDraw pointsofYcoordinate
 *              color: treatDraw pointsofcolor
 * @retval      none
 */
...
void lcd_show_char(uint16_t x, uint16_t y, char ch, lcd_font_t font, uint8_t mode, uint16_t color)
{
    ...
    for (byte_index = 0; byte_index < ch_size; byte_index++)
    {
        ...
        for (bit_index = 0; bit_index < 8; bit_index++)   /* 8 dots per byte */
        {
            ...
            {
                lcd_draw_point(x + width_index, y + height_index, color);           /* Draw pointscome out,To show this point */
            }
            else if (mode == 0)
            {
                lcd_draw_point(x + width_index, y + height_index, g_back_color);    /* paintingBackground color,Equivalent to thispointNot displayed(Note that the background color is controlled by global variables) */
            }
            ...
            if (height_index == ch_height)    /* One column has been displayed? */
            {
                ...
                width_index++;                /* xIncrement of coordinates */
                break;
            }
 ...
 *              mode : NUM_SHOW_NOZERO: High digit0Not displayed
...
void lcd_show_xnum(uint16_t x, uint16_t y, uint32_t num, uint8_t len,  lcd_font_t font, num_mode_t mode, uint16_t color)
{
    ...
    {
        num_index = (num / lcd_pow(10, len - len_index - 1)) % 10;    /* Get the corresponding number */
...
/**
 * @brief       LCDShow numbers，Not displayedhigh position0
 * @param       x    : treatShow numbersofXcoordinate
 *              y    : treatShow numbersofYcoordinate
 *              num  : treatShow numbers
 *              len  : The number of digits to be displayed
