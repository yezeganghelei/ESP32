...
typedef enum
{
  NUM_SHOW_NOZERO = 0x00,  /* Number high0Not displayed */
...
void lcd_clear(uint16_t color);                                                           /* LCD screen clear function */
...
void lcd_draw_point(uint16_t x, uint16_t y, uint16_t color);                                  /* lcdDraw points */
...
void lcd_show_xnum(uint16_t x, uint16_t y, uint32_t num, uint8_t len,  lcd_font_t font, num_mode_t mode, uint16_t color);   /* lcdDisplay numbers，Can control the display high position0 */
...
void lcd_show_pic(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t *pic);                                   /* lcdPicture display */
