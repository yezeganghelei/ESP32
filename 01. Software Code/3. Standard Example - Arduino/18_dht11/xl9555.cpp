...
void xl9555_write_reg(uint8_t reg, uint8_t data)
{
    ...
    Wire.endTransmission();                   /* IIC send Send queue data(Withoutparameter,expresssendstopSignal,end transfer) */          
}
...
uint8_t xl9555_read_reg(uint8_t reg)
{
    ...
    Wire.endTransmission(0);                  /* IIC send Send queue data(Transfer to the0,Indicates resending onestartSignal,KeepIICbus validconnect) */
...
/**
 * @brief       set upXL9555someoneIOThe mode(output or input)
