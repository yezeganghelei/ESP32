...
#define USE_LCD_BUF    1            /* Used by defaultlcd_buf,uselcd_bufThe screen-sinking speed will be greatly improved,Sacrifice space for speed,When there is not enough memory,set0Just */
#if USE_LCD_BUF
    #if SPI_LCD_TYPE
 ...
 * @note        generallyset upforL2R_U2DJust,ifset upFor other scanning methods,May cause abnormal display.
...
void lcd_init(void)
{
    ...
    /* LCDThe pins needed for the screenDefault stateforhigh level */
    ...
    spi_lcd->beginTransaction(SPISettings(SPICLK, MSBFIRST, SPI_MODE3));    /* Customized set used inSPISettings for SPIbusinitialization */
 ...
 *              color: treatpaintingrectangleboxofcolor
 * @retval      none
 */
...
void lcd_show_char(uint16_t x, uint16_t y, char ch, lcd_font_t font, uint8_t mode, uint16_t color)
{
    ...
    for (byte_index = 0; byte_index < ch_size; byte_index++)
    {
        ...
        for (bit_index = 0; bit_index < 8; bit_index++)   /* 8 points per byte */
        {
            ...
            {
                lcd_draw_point(x + width_index, y + height_index, g_back_color);    /* Drawing Background color is equivalent to this pointDon't show (Note that the background color is controlled by global variables) */
            }
...
/**
 * @brief       LCDDisplay numbers，Can control the display high position0
 ...
 * @note        Image modeling method: horizontal scan、RGB565、High in front
