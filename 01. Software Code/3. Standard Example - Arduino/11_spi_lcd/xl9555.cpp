...
uint8_t xl9555_read_reg(uint8_t reg)
{
    ...
    Wire.endTransmission(0);                  /* IIC send Send queue data(Pass parameters as0,Indicates resending astartSignal,KeepIICbus validconnect) */

    Wire.requestFrom(EXIO_ADDR, 1);           /* The host Towards the slave send data request, and get the data */
 ...
 * @param       portx : P0 / P1
 * @param       data  : IOThe status of(correspond8indivualIO)
 * @retval      none
 */
...
/**
 * @brief       readXL9555ofP0orP1portThe status of
 * @param       portx : P0 / P1
 * @retval      IOThe status of(correspond8indivualIO)
 */
...
void xl9555_io_config(uint16_t port_pin, io_mode_t mode)
{
    ...
    {
        config_value = config_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));   /* Get the ofPORT value after someoneIOset up is the output function but does not affect other IOThe status of which is not set upof */
    }
    else
    {
        config_value = config_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));      /* getsomeoneIOset as input functionPORTvalue but does not affect theset upofotherIOThe status of */
    }
...
void xl9555_pin_set(uint16_t port_pin, io_state_t state)
{
    ...
    {
        pin_value = pin_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));          /* getsomeoneIOAfter setting to high levelPORTvalue but does not affect theset upofotherIOThe status of */
    }
    else
    {
        pin_value = pin_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));       /* getsomeoneIOset upforLow levelbackofPORTvalue but does not affect theset upofotherIOThe status of */
    }
...
/**
 * @brief       GetXL9555Configured as input functionIOThe status of(High levelorLow level)
...
uint8_t xl9555_get_pin(uint16_t port_pin)
{
    ...
    pin_state = pin_state & port_value;                               /* getneedwantQueryBitThe status of */
