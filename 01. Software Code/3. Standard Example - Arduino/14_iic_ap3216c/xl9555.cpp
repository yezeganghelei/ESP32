...
void xl9555_io_config(uint16_t port_pin, io_mode_t mode)
{
    ...
    {
        config_value = config_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));   /* have toarrivesomeoneIOset upforOutput functionbackofPORTvalue but does not affect theset upofotherIOThe status of */
    }
    else
    {
        config_value = config_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));      /* have toarrivesomeoneIOSet as input functionPORTvalue but does not affect theset upofotherIOThe status of */
    }
...
void xl9555_pin_set(uint16_t port_pin, io_state_t state)
{
    ...
    {
        pin_value = pin_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));          /* Get someoneIOAfter setting to high levelPORT value but does not affect other IOThe status of is not set upof */
    }
    else
    {
        pin_value = pin_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));       /* have toarrivesomeoneIOset upforlow levelbackofPORTvalue but does not affect theset upofotherIOThe status of */
    }
