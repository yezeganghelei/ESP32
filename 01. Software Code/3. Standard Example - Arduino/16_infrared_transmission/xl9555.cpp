...
uint8_t xl9555_read_reg(uint8_t reg)
{
    ...
    Wire.endTransmission(0);                  /* IIC send Send the queue data(Pass parameters as0,Indicates resending astartSignal,KeepIIC bus valid connect) */
...
void xl9555_io_config(uint16_t port_pin, io_mode_t mode)
{
    ...
    {
        config_value = config_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));   /* have toarriveSomeIOset upforOutput functionbackofPORTvalue but does not affect theset upofotherIOThe status of */
    }
    else
    {
        config_value = config_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));      /* arriveSomeIOset as input functionPORTvalue but does not affect theset upofotherIOThe status of */
    }
...
void xl9555_pin_set(uint16_t port_pin, io_state_t state)
{
    ...
    {
        pin_value = pin_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));          /* have toarriveSomeIOAfter setting to high levelPORTvalue but does not affect theset upofotherIOThe status of */
    }
    else
    {
        pin_value = pin_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));       /* getarriveSomeIOset upforLow levelbackofPORTvalue but does not affect theset upofotherIOThe status of */
    }
...
uint8_t xl9555_get_pin(uint16_t port_pin)
{
    ...
    pin_state = pin_state & port_value;                               /* have toarriveneedwantQueryBitThe status of */
