...
#define USE_LCD_BUF    1            /* Used by defaultlcd_buf,uselcd_bufThe screen-sinking speed will be greatly improved,Sacrifice space to increase speed,When there is not enough memory,Set0That’s it */
#if USE_LCD_BUF
    #if SPI_LCD_TYPE
 ...
 * @param       xs: List the starting address
 ...
 * @param       dir:0~7,represent8One direction(See the specific definition forlcd.h)
 * @retval      none
 */
void lcd_scan_dir(uint8_t dir)
{
    ...
    if (spilcd_dir == 1)  /* Horizontal screen，Need to change scanning direction！ */
    {
...
void lcd_init(void)
{
    ...
    lcd_write_cmd(0x21);        /* Reverse display */
 ...
 *              color: The color of the round frame to be drawn
 * @retval      none
 */
...
void lcd_show_char(uint16_t x, uint16_t y, char ch, lcd_font_t font, uint8_t mode, uint16_t color)
{
    ...
    for (byte_index = 0; byte_index < ch_size; byte_index++)
    {
        ...
        for (bit_index = 0; bit_index < 8; bit_index++)   /* 8 dots per byte */
        {
            ...
            byte_code <<= 1;                  /* Shift, In order to obtain the state of the next bit */
        }
    }
}
...
/**
 * @brief       LCDShow numbers，Can control the display high position0
 ...
 * @note        Image modeling method: horizontal scan、RGB565、High position first
