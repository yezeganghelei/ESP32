...
#define XL9555_INVERSION_PORT0_REG    4           /* Polarity inversion register：used whenP0When the port is used as input，Invert the input level，That is, when the pin is high level，set upThis register corresponds toofThe bit is1hour，Readarriveofinput register0，1The value is low level0 */
#define XL9555_INVERSION_PORT1_REG    5           /* Polarity inversion register: used whenP1When the port is used as input, Invert the input level, That is, when the pin is high level, set up the corresponding of bit in this register is 1hour, Read to ofinput register0, 1The value is low level0 */
