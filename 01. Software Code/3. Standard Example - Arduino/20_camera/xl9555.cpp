...
void xl9555_write_reg(uint8_t reg, uint8_t data)
{
    ...
    Wire.endTransmission();                   /* IIC send sendqueueofdata(Withoutparameter,expresssendstopSignal,End transmission) */          
}
...
uint8_t xl9555_read_reg(uint8_t reg)
{
    ...
    Wire.endTransmission(0);                  /* IIC send sendqueueofdata(Pass parameters as0,Indicates resending astartSignal,KeepIICbus active connection) */
