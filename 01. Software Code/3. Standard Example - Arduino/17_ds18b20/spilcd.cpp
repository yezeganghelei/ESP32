...
uint8_t spilcd_dir = 1;             /* Default horizontal screen(1)、Vertical screen(0) */

#define USE_LCD_BUF    1            /* Used by defaultlcd_buf,uselcd_bufThe screen-sinking speed will be greatly improved,Sacrifice space to increase speed,When memory is insufficient,Set0That’s it */
#if USE_LCD_BUF
    #if SPI_LCD_TYPE
...
static const int SPICLK = 80000000; /* SPI communication rate (screen display abnormality, lower SPICLK) */
...
/**
 * @brief	      PastLCDWrite a specified amount of data
...
/**
 * @brief       set upLCDRow and column address
 ...
 * @note        generallyset upforL2R_U2DThat’s it,ifset upFor other scanning methods,It may cause abnormal display.
 * @param       dir:0~7,represent8direction(For specific definitions, seelcd.h)
 * @retval      none
 */
void lcd_scan_dir(uint8_t dir)
{
    ...
    if (spilcd_dir == 1)  /* When screening horizontally，Need to change the scanning direction！ */
    {
    ...
    /* Set the value of bit 5, 6, and 7 of register 0x36 according to the scanning mode. */
    switch (dir)
    {
...
void lcd_init(void)
{
    /* initializationLCDThe pins required for the screen */
    xl9555_io_config(SLCD_PWR, IO_SET_OUTPUT);
    xl9555_io_config(SLCD_RST, IO_SET_OUTPUT);
    pinMode(SLCD_WR_PIN, OUTPUT);

    /* LCDThe pins required for the screenThe default state is high */
    ...
    spi_lcd->begin(SLCD_SCK_PIN, SLCD_SDI_PIN, SLCD_SDA_PIN, SLCD_CS_PIN);  /* set upSPICommunication line */
    ...
    spi_lcd->beginTransaction(SPISettings(SPICLK, MSBFIRST, SPI_MODE3));    /* Used inSPISettingsCustomize inofmatchSetconductSPIbusinitialization */
    
    LCD_CS(0);                  /* Pull down the selection line,Select device */

    /* rightLCDofregisterConfigure */
#if SPI_LCD_TYPE                /* right2.4inchLCDregisterconductset up */
...
#else                           /* right1.3inchLCDregisterconductset up */
    lcd_write_cmd(0x11);        /* Sleep Out */
    delay(120);                 /* wait for power stability */
 ...
 * @param       xs   : region startXcoordinate
 *              ys   : region startYcoordinate
...
void lcd_fill(uint16_t xs, uint16_t ys, uint16_t xe, uint16_t ye, uint16_t color)
{
    ...
    area_size = (xe - xs + 1) * (ye - ys + 1) * sizeof(uint16_t);   /* Calculate the number of bytes in the area */
...
/**
 * @brief       LCDDraw line segments
 * @param       x1   : treatDraw line segmentsendpoint1ofXcoordinate
 *              y1   : treatDraw line segmentsendpoint1ofYcoordinate
 *              x2   : treatDraw line segmentsendpoint2ofXcoordinate
 *              y2   : treatDraw line segmentsendpoint2ofYcoordinate
 ...
 * @param       x1   : End point of the rectangle frame to be drawn1ofXcoordinate
 *              y1   : End point of the rectangle frame to be drawn1ofYcoordinate
 *              x2   : End point of the rectangle frame to be drawn2ofXcoordinate
 *              y2   : End point of the rectangle frame to be drawn2ofYcoordinate
 ...
 *              font : Font of the characters to be displayed
...
void lcd_show_char(uint16_t x, uint16_t y, char ch, lcd_font_t font, uint8_t mode, uint16_t color)
{
    ...
    ch_offset = ch - ' ';   /* Get the offset value (ASCII font library starts with spaces, so -' ' is the font library for the corresponding characters) */
    ...
    for (byte_index = 0; byte_index < ch_size; byte_index++)
    {
        ...
        for (bit_index = 0; bit_index < 8; bit_index++)   /* 8 dots per byte */
        {
            ...
            {
                lcd_draw_point(x + width_index, y + height_index, color);           /* draw dotscome out,To show this point */
            }
            else if (mode == 0)
            {
                lcd_draw_point(x + width_index, y + height_index, g_back_color);    /* Drawing Background color is equivalent to One dotDon't show (Note that the background color is controlled by global variables) */
            }
 ...
 * @note will automatically wrap lines and pages
 * @param x: X coordinate of the string to be displayed
 * y: Y coordinate of the string to be displayed
 * str: string to be displayed
 * font: the font of the string to be displayed
 * color: color of the string to be displayed
 * @retval None
 */
...
/**
 * @brief       LCDDisplay numbers，Controllable display high bit0
 * @param       x    : treatDisplay numbersofXcoordinate
 *              y    : treatDisplay numbersofYcoordinate
 *              num  : treatDisplay numbers
 *              len  : Number of digits to be displayed
 *              mode : NUM_SHOW_NOZERO: High digit0Don't show
 *                     NUM_SHOW_ZERO  : High digit0show
 *              font : The font of the numbers to be displayed
...
void lcd_show_xnum(uint16_t x, uint16_t y, uint32_t num, uint8_t len,  lcd_font_t font, num_mode_t mode, uint16_t color)
{
    ...
    for (len_index = 0; len_index < len; len_index++)                 /* Cycling by total number of displayed digits */
    {
        num_index = (num / lcd_pow(10, len - len_index - 1)) % 10;    /* Get the number of the corresponding bit */
        if ((first_nozero == 0) && (len_index < (len - 1)))           /* There is no enabled display, and there are bits to display */
        {
            if (num_index == 0)
            {
...
/**
 * @brief       LCDDisplay numbers，Don't showhigh position0
 * @param       x    : treatDisplay numbersofXcoordinate
 *              y    : treatDisplay numbersofYcoordinate
 *              num  : treatDisplay numbers
 *              len  : Number of digits to be displayed
...
/**
 * @brief       LCDImage display
 * @note        Picture taking mode: Horizontal scanning、RGB565、High position first
 * @param       x     : Picture to be displayedXcoordinate
 *              y     : Picture to be displayedYcoordinate
 *              width : Width of the picture to be displayed
 *              height: Picture to be displayedhigh
