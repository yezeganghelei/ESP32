...
/**
* @brief       initializationIOExpanding chip
* @param       none
* @retval      none
*/
void xl9555_init(void)
{
    pinMode(IIC_INT_PIN, INPUT_PULLUP);     /* Configure interrupt pin */

    Wire.begin(IIC_SDA, IIC_SCL, 400000);   /* Initialize IIC connection */

    /* Power on firstReadClear interrupt flag once */
    xl9555_read_port(0);
    xl9555_read_port(1);
}
...
void xl9555_write_reg(uint8_t reg, uint8_t data)
{
    ...
    Wire.write(reg);                          /* sendwantwriteSlave machineregisterofaddressarrivesendqueue */
    Wire.write(data);                         /* Send data to be written to the slave register to the send queue */
    Wire.endTransmission();                   /* IIC send Send queue data(Withoutparameter,expresssendstopSignal,End transmission) */          
}

/**
 * @brief       TowardsXL9555Read data from relevant registers
 * @param       reg    : Register address
 * @retval      registerofvalue / 0xFF:No data received
 */
uint8_t xl9555_read_reg(uint8_t reg)
{
    Wire.beginTransmission(EXIO_ADDR);        /* sendSlave machineof7Bit device address to send queue */
    Wire.write(reg);                          /* send requires Read slave ofRegister address to send queue */
    Wire.endTransmission(0);                  /* IIC send Send queue data(Transfer to the0,Indicates resending astartSignal,KeepIIC bus valid connect) */
...
/**
 * @brief ReadXL9555ofP0orP1 port status
 * @param portx: P0 / P1
...
/**
 * @brief set upXL9555someoneIOmode(Output or input)
...
void xl9555_io_config(uint16_t port_pin, io_mode_t mode)
{
    ...
    config_reg  = xl9555_read_reg(port_pin > XL_PORT0_ALL_PIN ? XL9555_CONFIG_PORT1_REG : XL9555_CONFIG_PORT0_REG);   /* FirstReadset upPinWhereofregisterCondition */

    if (mode == IO_SET_OUTPUT)    /* According to modeparameter set up input and output conditions, cannot affect otherIO */
    {
        config_value = config_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));   /* have toarrivesomeoneIOset upforOutput functionbackofPORTvalue but does not affect theset upofotherIOstatus */
    }
    else
    {
        config_value = config_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));      /* arrivesomeoneIOSet as input functionPORTvalue but does not affect theset upofotherIOstatus */
    }
...
void xl9555_pin_set(uint16_t port_pin, io_state_t state)
{
    ...
    pin_reg = xl9555_read_reg(port_pin > XL_PORT0_ALL_PIN ? XL9555_OUTPUT_PORT1_REG : XL9555_OUTPUT_PORT0_REG);     /* FirstReadset upPinWhereofregisterCondition */

    if (state == IO_SET_HIGH)    /* according to stateparameter set upIOofHighLow level */
    {
        pin_value = pin_reg | (port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0));          /* have toarrivesomeoneIOAfter setting to high levelPORTvalue but does not affect theset upofotherIOstatus */
    }
    else
    {
        pin_value = pin_reg & (~(port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0)));       /* have toarrivesomeoneIOset upforLow levelbackofPORTvalue but does not affect theset upofotherIOstatus */
    }

    xl9555_write_reg(port_pin > XL_PORT0_ALL_PIN ? XL9555_OUTPUT_PORT1_REG : XL9555_OUTPUT_PORT0_REG, pin_value);   /* Set the IO high and low level status to the output register */
}
...
uint8_t xl9555_get_pin(uint16_t port_pin)
{
    ...
    pin_state = port_pin >> (port_pin > XL_PORT0_ALL_PIN ? 8 : 0);    /* If soPORT1ofPINNeed to move right first8Bit */
    pin_state = pin_state & port_value;                               /* have toarriveneedwantQueryBitstatus */
