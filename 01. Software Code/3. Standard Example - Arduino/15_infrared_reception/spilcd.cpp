...
uint16_t spilcd_width  = 240;       /* Screen width 240 (vertical screen) */
uint16_t spilcd_height = 320;       /* screen width 320(Vertical screen) */
#else
uint16_t spilcd_width  = 240;       /* Screen width 240 (vertical screen) */
uint16_t spilcd_height = 240;       /* Screen width 240 (vertical screen) */
#endif                              /* 1.3 inch SPI_LCD screen */

uint8_t spilcd_dir = 1;             /* Default landscape(1)、Vertical screen(0) */

#define USE_LCD_BUF    1            /* Used by defaultlcd_buf,uselcd_bufThe refresh speed will be greatly improved,Sacrifice space to increase speed,When memory is insufficient,Set0That’s it */
#if USE_LCD_BUF
    #if SPI_LCD_TYPE
...
void lcd_show_char(uint16_t x, uint16_t y, char ch, lcd_font_t font, uint8_t mode, uint16_t color)
{
    ...
    for (byte_index = 0; byte_index < ch_size; byte_index++)
    {
        ...
        for (bit_index = 0; bit_index < 8; bit_index++)   /* 8 dots per byte */
        {
            ...
            {
                lcd_draw_point(x + width_index, y + height_index, g_back_color);    /* Draw Background color, which is equivalent to One dotNot displayed (Note that the background color is controlled by global variables) */
            }

            height_index++;

            if (height_index == ch_height)    /* A column has been displayed? */
            {
