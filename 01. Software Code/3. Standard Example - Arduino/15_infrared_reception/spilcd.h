...
/* LCDDisplay numbersMode enumeration */
typedef enum
{
  NUM_SHOW_NOZERO = 0x00,  /* High digit0Not displayed */
...
/* The following twoSPI_LCDNeed to useIOexistxl9555.halready indefinition
 * #define SLCD_PWR_PIN          
 * #define SLCD_RST_PIN  
 */
...
#define R2L_U2D         2           /* right to left,from top to bottom */
#define R2L_D2U         3           /* right to left,from bottom to top */
...
void lcd_set_address(uint16_t xs, uint16_t ys, uint16_t xe, uint16_t ye);                 /* set uplcdRow and queue address */
void lcd_clear(uint16_t color);                                                           /* lcd screen clearing function */
void lcd_display_on(void);                                                                /* turn onlcdBacklight */
...
/* lcdFunction function */
void lcd_draw_point(uint16_t x, uint16_t y, uint16_t color);                                  /* lcddraw dots */
void lcd_draw_line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color);       /* lcdDraw line segments */
void lcd_draw_hline(uint16_t x, uint16_t y, uint16_t len, uint16_t color);                    /* lcdDraw horizontal lines */
void lcd_draw_rectangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color);  /* LCD draws rectangular frame */
void lcd_draw_circle(uint16_t x, uint16_t y, uint16_t r, uint16_t color);                     /* LCD draw a circular frame */
...
void lcd_show_xnum(uint16_t x, uint16_t y, uint32_t num, uint8_t len,  lcd_font_t font, num_mode_t mode, uint16_t color);   /* lcdDisplay numbers，Controllable display high bit0 */
void lcd_show_num(uint16_t x, uint16_t y, uint32_t num, uint8_t len, lcd_font_t font, uint16_t color);                      /* lcdDisplay numbers，Not displayedhigh position0 */
